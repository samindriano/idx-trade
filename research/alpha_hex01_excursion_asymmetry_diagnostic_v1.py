"""Outcome-blind structural diagnostic for H-EXC-01 excursion asymmetry."""
...
SCORE = "H_EXC_01_excursion_asymmetry_5_v1"
...
def build_scores(full: pd.DataFrame) -> pd.DataFrame:
    ...
    up_excursion = result["high"] - result["prev_close"]
    down_excursion = result["prev_close"] - result["low"]
    denominator = up_excursion + down_excursion
    result["daily_excursion_asym"] = ((up_excursion - down_excursion) / denominator).where(
        valid & denominator.gt(0)
    )
    result[SCORE] = rolling(result, "daily_excursion_asym", 5, "median")
    ...
    short = rolling(result, "range_pct", 5, "median")
    long = rolling(result, "range_pct", 60, "median")
    ratio = short / long
...
def metric_block(frame: pd.DataFrame, score: str, other_sets: dict[str, dict[pd.Timestamp, set[str]]]) -> dict[str, object]:
    sets = top30_sets(frame, score)
    selected = frame[["ticker", "date", "eligible_decision_universe", score, "regular_market_value", "dollar_turnover"]]
    ...
    return {
        ...
        "top30_overlap": {label: overlap_stats(sets, values) for label, values in other_sets.items()},
        "top30_sets": sets,
    }
...
def main() -> None:
    ...
    rank_dependence = {
        ...
        for label, column in {
            "C1": CANDIDATES[0], "C2": CANDIDATES[1], "C4": CANDIDATES[2],
            "H-LIQ-01": HLIQ, "H-VOL-01_5v60": HVOL,
        }.items()
    }
    metrics = metric_block(scored, SCORE, comparison_sets)
    metrics.pop("top30_sets")
    result = {
        "status": "PASS_STRUCTURAL_ONLY",
        "stage": "E_F_HEXC01_OUTCOME_BLIND_EXCURSION_ASYMMETRY_DIAGNOSTIC",
        ...
        "formula": {
            "name": SCORE,
            "daily_term": "((high-prev_close)-(prev_close-low))/((high-prev_close)+(prev_close-low))",
            "aggregation": "median",
            "window_sessions": 5,
            "direction": "upside_excursion_dominance",
            "uses_open": False,
            "uses_volume": False,
        },
        "metrics": metrics,
        "daily_rank_dependence": rank_dependence,
        "interpretation": {
