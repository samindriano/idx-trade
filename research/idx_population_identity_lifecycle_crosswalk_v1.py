...
def build_crosswalk(args: argparse.Namespace) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    packet = {
        ...
        "authority_classification_summary": {
            "IDENTITY_PROVEN": 0,
            "IDENTITY_BOUNDED": sum(row["identity_status"] == "IDENTITY_BOUNDED" for row in rows),
            "LIFECYCLE_PROVEN": 0,
            "LIFECYCLE_PARTIAL": sum(row["lifecycle_status"] == "LIFECYCLE_PARTIAL" for row in rows),
            "LIFECYCLE_CONFLICTING": sum(row["lifecycle_status"] == "CONFLICTING" for row in rows),
            "PIT_EFFECTIVE_DATE_PROVEN": 0,
            "PIT_EFFECTIVE_DATE_UNKNOWN": len(rows),
            "POPULATION_COMPLETE": 0,
            "POPULATION_INCOMPLETE": 0,
            "POPULATION_UNKNOWN": len(rows),
            "interpretation": "Population completeness is not assigned as complete or incomplete; the retained union is observed-only and its historical denominator is unknown.",
        },
        ...
        "final_coverage_metrics": {
            "observed_research_population_codes": len(observed_union),
            "ksei_population_attempted_codes": len(ksei_codes),
            "ksei_resolved_codes": sum(detail.get("parse_status") == "DETAIL_VALID" for detail in ksei_details.values()),
            "isin_resolved_codes": sum(bool(detail.get("isin")) and detail.get("parse_status") == "DETAIL_VALID" for detail in ksei_details.values()),
            "issuer_name_evidence_codes": sum(not row["residual_flags"]["no_issuer"] for row in rows),
            "issuer_legal_continuity_proven_codes": 0,
            "lifecycle_interval_or_event_codes": len(set(intervals) | set(issued)),
            "lifecycle_proven_codes": 0,
            "lifecycle_partial_codes": sum(row["lifecycle_status"] == "LIFECYCLE_PARTIAL" for row in rows),
            "lifecycle_conflicting_codes": len(lifecycle_meta["conflict_codes"]),
            "current_listing_codes": len(current_codes),
            "delisting_codes": len(delisting_codes),
            "relisting_proven_codes": 0,
            "historical_only_delisting_codes": len(source_only_sets.get("official_delisting_records", set())),
            "official_corporate_action_sample_codes": len(ca_codes),
        },
