"""Outcome-blind H-EXC-02 sensitivity to retained unresolved CA rows."""
...
from alpha_hex02_bounded_excursion_balance_diagnostic_v1 import SCORE
from alpha_hvol01_compression_diagnostic_v1 import top30_sets
...
def compute_score(panel: pd.DataFrame, universe: pd.DataFrame) -> pd.DataFrame:
    ...
    full["prev_close"] = full.groupby("ticker", sort=False)["close"].shift(1)
    valid = (
        full["high"].gt(0)
        & full["low"].gt(0)
        & full["prev_close"].gt(0)
        & np.isfinite(full["high"])
        & np.isfinite(full["low"])
        & np.isfinite(full["prev_close"])
        & full["high"].ge(full["low"])
    )
    up_distance = (full["high"] - full["prev_close"]).abs()
    down_distance = (full["low"] - full["prev_close"]).abs()
    denominator = up_distance + down_distance
    full["bounded_daily_balance"] = ((up_distance - down_distance) / denominator).where(
        valid & denominator.gt(0)
    )
    full[SCORE] = rolling(full, "bounded_daily_balance", 5, "median")
...
def main() -> None:
    ...
    for name in ("panel", "features", "sessions", "anchors", "unresolved-scale", "manifest", "hex02-output", "output"):
    ...
    result = {
        ...
        "formula": {
            ...
            "aggregation": "median",
            "window_sessions": 5,
            "counterfactual": "close replaced by retained idx_close at 188 unresolved keys in memory only",
        },
