"""Prepare one outcome-blind E2E PAPER execution with dual-calendar proof."""
...
from idx_trade.e2e_paper_schedule_binding_v1 import write_prepared_schedule_binding
...
def _run(args: argparse.Namespace) -> int:
    ...
    # POST_EOD is anchored to the observed session that actually closed today.
    require_phase_window(
        phase="POST_EOD",
        session_date=eod.session_date,
    ...
    binding = write_prepared_schedule_binding(
        args.runtime_root,
        prepared_path=result.path,
        eod_inputs=eod,
    )
    print(
        {
            ...
            "schedule_binding_path": str(binding.path),
