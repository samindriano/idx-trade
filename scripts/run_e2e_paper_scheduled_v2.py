"""Run the dual-calendar E2E PAPER controller from pinned external config."""
...
_CONTRACT_VERSION = "DUAL_CALENDAR_V1"
...
def _bootstrap_attest(runtime_root: Path, expected_sha256: str | None) -> str:
    ...
    if payload.get("operational_contract_version") != _CONTRACT_VERSION:
        raise _BootstrapError("E2E_RUNTIME_CONFIG_DUAL_CALENDAR_CONTRACT_MISSING")
    ...
    schedule_path = Path(str(payload.get("execution_schedule_attestation_path") or "")).expanduser()
    schedule_sha = str(payload.get("execution_schedule_attestation_sha256") or "").strip().lower()
    if not schedule_path.is_absolute() or not _SHA_RE.fullmatch(schedule_sha):
        raise _BootstrapError("E2E_RUNTIME_CONFIG_EXECUTION_SCHEDULE_IDENTITY_INVALID")
...
def main() -> int:
    ...
    try:
        ...
        print(
            json.dumps(
                {"controller_status": "WAITING_OPERATIONAL_CONFIGURATION", "reason": str(exc)},
                sort_keys=True,
            )
        )
        return 1
    try:
        ...
        from idx_trade.e2e_paper_operational_controller_v2 import run_operational_cycle_v2
