...
SCHEMA = "inc001_ca_economic_event_reconciliation_v2"
PROJECT_ROOT = Path(r"D:\Documents\Project")
PRIOR_ACQUISITION_ROOT = PROJECT_ROOT / "idx-ca-stock-split-acquisition-20260829-v1"
NEW_ACQUISITION_ROOT = PROJECT_ROOT / "idx-ca-stock-split-acquisition-20260829-v3"
...
V3_MANIFEST_SHA256 = "60d4b5caf9fbadd81c8f63edf4976f2d476ead6a26884c9d74f965759250a746"
...
def new_wave_document_inventory() -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    entries = json.loads((NEW_ACQUISITION_ROOT / "provider" / "document_request_ledger.json").read_text(encoding="utf-8"))
    documents: list[dict[str, Any]] = []
    parsed: list[dict[str, Any]] = []
    for number, entry in enumerate(entries, start=1):
        document = base.parse_document(entry, NEW_ACQUISITION_ROOT, f"ACQ-V2-DOC-{number:03d}")
        if document is None:
            continue
        title = text(entry.get("title"))
        for candidate_ticker in ("HEAL", "SCMA"):
            if candidate_ticker in title.upper():
                document["ticker"] = candidate_ticker
                break
...
def load_document_sets() -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], dict[str, dict[str, Any]]]:
    old_docs, old_parsed = base.document_inventory(PRIOR_ACQUISITION_ROOT)
    new_docs, new_parsed = new_wave_document_inventory()
    ...
    old_target_docs, _ = base.target_document_map(source, old_parsed, probe_parsed, PRIOR_ACQUISITION_ROOT)
    current_results = read_csv(NEW_ACQUISITION_ROOT / "target_event_results.csv")
    current_targets = {text(row.get("event_id")): row for row in current_results if text(row.get("result_classification")) == "RESOLVED_EXACT"}
    current_docs: dict[str, dict[str, Any]] = {}
    for event_id, target in current_targets.items():
        candidates = [doc for doc in new_parsed if text(doc.get("ticker")) == text(target.get("ticker"))]
        if not candidates:
            raise RuntimeError(f"resolved acquisition result has no parsed document: {event_id}")
        candidates.sort(key=lambda row: (text(row.get("document_id")), text(row.get("evidence_sha256"))))
        chosen = dict(candidates[0])
        chosen["target_event_id"] = event_id
        current_docs[text(target.get("source_event_id"))] = chosen
    return [*old_docs, *new_docs], [*old_parsed, *new_parsed], list(probe_docs), {**old_target_docs, **current_docs}
...
def future_plan(
    ...
    resolved_wave = sorted(text(row.get("event_id")) for row in target_results if text(row.get("result_classification")) == "RESOLVED_EXACT")
...
def build(output_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        linkages = base.build_linkages(source, target_docs, probe_docs)
        ...
        prior = {"source_evidence_rows": 412, "cross_source_collapses": 20, "same_source_collapses": 3, "economic_event_count": 389, "resolved_transitions": 153, "unresolved_transitions": 190, "non_basis_excluded": 46}
        ...
        transition_target_ids = {text(row.get("source_event_id")) for row in target_results if text(row.get("result_classification")) == "RESOLVED_EXACT"}
        ...
        validation = {
            ...
            "new_wave_bbrm_semantic_insufficient": sum(text(row.get("ticker")) == "BBRM" and text(row.get("result_classification")) == "DOCUMENT_FOUND_SEMANTIC_INSUFFICIENT" for row in target_results),
        ...
        summary = {
            ...
            "controlling_predecessor_manifest_sha256": V3_MANIFEST_SHA256,
            "controlling_v4_manifest_sha256": V4_MANIFEST_SHA256,
            "source_authority_manifest_sha256": SOURCE_MANIFEST_SHA256,
            "acquisition": {"new_root": str(NEW_ACQUISITION_ROOT), "new_manifest_sha256": sha256_file(NEW_ACQUISITION_ROOT / "MANIFEST.json"), "prior_root_reused": str(PRIOR_ACQUISITION_ROOT), "provider_calls_in_reconciliation": False},
            "counts": actual,
            "before_after_comparison": comparison,
            "new_wave": {"target_count": len(target_results), "resolved_exact_targets": len(resolved_target_transitions), "bbrm_result": "DOCUMENT_FOUND_SEMANTIC_INSUFFICIENT", "new_transition_evidence_count": len(resolved_target_transitions)},
        ...
        write_json(staging / "deterministic_input_pins.json", {"source_authority_root": str(PROJECT_ROOT / "idx-ca-source-authority-audit-20260829-v11-deterministic-rerun-v8"), "source_authority_manifest_sha256": SOURCE_MANIFEST_SHA256, "prior_reconciliation_root": str(PROJECT_ROOT / "idx-ca-economic-event-reconciliation-20260829-v3"), "prior_reconciliation_manifest_sha256": V3_MANIFEST_SHA256, "v4_decomposition_root": str(PROJECT_ROOT / "idx-ca-unresolved-economic-gap-decomposition-20260829-v4"), "v4_decomposition_manifest_sha256": V4_MANIFEST_SHA256, "prior_acquisition_root": str(PRIOR_ACQUISITION_ROOT), "new_acquisition_root": str(NEW_ACQUISITION_ROOT), "new_acquisition_manifest_sha256": sha256_file(NEW_ACQUISITION_ROOT / "MANIFEST.json"), "probe_root": str(PROBE_ROOT), "repo_head": subprocess.check_output(["git", "rev-parse", "HEAD"], text=True).strip()})
