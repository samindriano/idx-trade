...
from .ranking_v3_structure_lite import V3_B_CANDIDATE, V3_B_FEATURE_COLUMNS
...
def _assert_cache(
    ...
    if manifest.get("status") != V4_A_CACHE_STATUS:
        raise RuntimeError("V4-A cache manifest is not frozen pre-outcome")
    if bool(manifest.get("outcome_metrics_computed", True)):
        raise RuntimeError("V4-A cache manifest unexpectedly contains outcome metrics")
    if bool(manifest.get("post_1224_materialized", True)):
        raise RuntimeError("V4-A cache manifest claims session 1225+ materialization")
    if bool(manifest.get("fresh_forward_accessed", True)):
        raise RuntimeError("V4-A cache manifest claims fresh-forward access")
    if bool(manifest.get("integration_candidate_materialized", True)):
        raise RuntimeError("V4-A first-pass cache claims integration candidate materialization")
    if manifest.get("spec_git_blob") != spec_blob:
        raise RuntimeError("V4-A cache manifest spec identity mismatch")

    actual_cache_sha = sha256_file(cache_path)
    if actual_cache_sha != manifest.get("cache_sha256"):
        raise RuntimeError("V4-A cache SHA mismatch")
    ...
    required = {
        ...
        "binary_target",
        *candidate_feature_columns(V4_A1_CANDIDATE),
        *candidate_feature_columns(V4_A2_CANDIDATE),
    }
    missing = required - set(table.columns)
    if missing:
        raise RuntimeError(f"V4-A cache missing required columns: {sorted(missing)}")
    if table.duplicated(["ticker", "date"]).any():
        raise RuntimeError("V4-A cache contains duplicate ticker/date rows")
...
def _read_reference(
    *,
    ...
    if hashes != expected:
        raise RuntimeError(
            f"V4-A frozen V3-B reference hash mismatch: expected={expected} actual={hashes}"
        )
...
def _score_candidate(
    ...
    *,
    ...
    if candidate not in V4_A_FIRST_PASS_CANDIDATES:
        raise ValueError(f"unknown V4-A first-pass candidate: {candidate}")
...
def _normalize_identity(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise RuntimeError(f"V4-A prediction identity missing columns: {sorted(missing)}")
...
def prove_v3_b_control_equivalence(
    *,
    ...
    if len(control_predictions) != len(reference_predictions):
        raise RuntimeError("V4-A control row count differs from frozen V3-B reference")
    if not _normalize_identity(control_predictions).equals(_normalize_identity(reference_predictions)):
        raise RuntimeError("V4-A control row identity/order differs from frozen V3-B reference")
    ...
    if not np.allclose(new_score, ref_score, rtol=0.0, atol=CONTROL_SCORE_ATOL, equal_nan=False):
        raise RuntimeError(
            "V4-A exact V3-B score equivalence failed: "
    ...
    for column in METRIC_COLUMNS:
        ...
        if not np.allclose(a, b, rtol=0.0, atol=METRIC_ATOL, equal_nan=False):
            raise RuntimeError(f"V4-A V3-B metric equivalence failed {column}: max={maximum}")
    return {
...
def _gate(candidate_metrics: pd.DataFrame, control_metrics: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any], bool]:
    candidate = candidate_metrics.set_index("fold").loc[list(FOLD_NAMES)]
    required = [*METRIC_COLUMNS]
    finite = bool(
        np.isfinite(candidate[required].apply(pd.to_numeric, errors="raise").to_numpy(dtype=float)).all()
    )
...
def _top_decile_overlap(control_predictions: pd.DataFrame, candidate_predictions: pd.DataFrame) -> pd.DataFrame:
    ...
    for fold in FOLD_NAMES:
        ...
        if not control[identity].equals(candidate[identity]):
            raise RuntimeError(f"V4-A top-decile identity mismatch for {fold}")
...
def run_v4a_first_pass(
    *,
    ...
    """Atomic first-pass V4-A run: exact V3-B control + A1 + A2, no integration."""
    ...
    control_metrics, control_predictions, model_hashes = _score_candidate(
        table, V4_A_CONTROL, output_dir=control_dir
    )
    elapsed[V4_A_CONTROL] = time.perf_counter() - t0
    ...
    equivalence_path = output_dir / "ranking_v4_a_v3_b_control_equivalence.json"
    ...
    for candidate, dirname in (
        (V4_A1_CANDIDATE, "a1_impact"),
        (V4_A2_CANDIDATE, "a2_persistent_direction"),
    ):
        candidate_dir = output_dir / dirname
        candidate_dir.mkdir(parents=True, exist_ok=False)
        t0 = time.perf_counter()
        metrics, predictions, model_hashes = _score_candidate(
            table, candidate, output_dir=candidate_dir
        )
    ...
    for candidate in (V4_A1_CANDIDATE, V4_A2_CANDIDATE):
    ...
    metrics_path = output_dir / "ranking_v4_a_f1_f6_metrics.csv"
    predictions_path = output_dir / "ranking_v4_a_f1_f6_predictions.parquet"
    paired_path = output_dir / "ranking_v4_a_f1_f6_paired.csv"
    overlap_path = output_dir / "ranking_v4_a_f1_f6_top_decile_overlap.csv"
    verdict_path = output_dir / "ranking_v4_a_f1_f6_verdict.json"
    runtime_path = output_dir / "ranking_v4_a_f1_f6_runtime.json"
    ...
    verdict = {
        ...
        "hypothesis_id": "V4-A-PARTICIPATION-V1",
        "control": V4_A_CONTROL,
        "challengers": results,
        "survivors": survivors,
        "integration_authorized_by_result": bool(len(survivors) == 2),
        "integration_executed": False,
        "candidate_ordinals_viewed": [12, 13, 14],
        "cumulative_historical_candidate_count_after_run": 12,
    ...
    summary = {
        ...
        "hypothesis_id": "V4-A-PARTICIPATION-V1",
        "candidate_ids": list(V4_A_FIRST_PASS_CANDIDATES),
        "folds": list(FOLD_NAMES),
    ...
    summary_path = output_dir / "ranking_v4_a_f1_f6_summary.json"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run frozen V4-A A1/A2 first-pass historical-development comparison")
