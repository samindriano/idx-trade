...
def classify_live_probe(rows: pd.DataFrame, config: dict[str, Any]) -> str:
    ...
    if len(qualifying) > 0:
        return "TV60_OPEN_BOUNDARY_PATTERN_FOUND_MEANING_UNPROVEN"
    # A regular-session pre-open contradiction is not evidence that the
    # extended feed contains an opening auction.  The pinned provider payload
    # has no auction flag or trade classification, so all contradictory
    # boundary observations remain fail-closed and meaning-unproven.
