...
def _assert_cache(
    ...
    if manifest.get("status") != V4_C_CACHE_STATUS:
        raise RuntimeError("V4-C cache manifest is not frozen pre-outcome")
    if bool(manifest.get("outcome_metrics_computed", True)):
        raise RuntimeError("V4-C cache manifest unexpectedly contains outcome metrics")
    if bool(manifest.get("post_1224_materialized", True)):
        raise RuntimeError("V4-C cache manifest claims session 1225+ materialization")
    if bool(manifest.get("fresh_forward_accessed", True)):
        raise RuntimeError("V4-C cache manifest claims fresh-forward access")
    if bool(manifest.get("integration_candidate_materialized", True)):
        raise RuntimeError("V4-C cache manifest claims an integration candidate")
    ...
    if manifest.get("spec_git_blob") != spec_blob:
        raise RuntimeError("V4-C cache manifest spec identity mismatch")

    actual_cache_sha = sha256_file(cache_path)
    if actual_cache_sha != manifest.get("cache_sha256"):
        raise RuntimeError("V4-C cache SHA mismatch")
    ...
    required = {
        ...
        "binary_target",
        *candidate_feature_columns(V4_C_CHALLENGER),
    }
    missing = required - set(table.columns)
    if missing:
        raise RuntimeError(f"V4-C cache missing required columns: {sorted(missing)}")
    if table.duplicated(["ticker", "date"]).any():
        raise RuntimeError("V4-C cache contains duplicate ticker/date rows")
...
def _read_reference(
    *,
    ...
    if hashes != expected:
        raise RuntimeError(
            f"V4-C frozen V3-B reference hash mismatch: expected={expected} actual={hashes}"
        )
    ...
    metrics = pd.concat(
        [metrics[metrics["fold"].eq(name)] for name in FOLD_NAMES], ignore_index=True
    )
    predictions = pd.concat(
...
def _score_candidate(
    ...
    *,
    ...
    if candidate not in V4_C_FIRST_PASS_CANDIDATES:
        raise ValueError(f"unknown V4-C first-pass candidate: {candidate}")
...
def _normalize_identity(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise RuntimeError(f"V4-C prediction identity missing columns: {sorted(missing)}")
...
def prove_v3_b_control_equivalence(
    *,
    ...
    if len(control_predictions) != len(reference_predictions):
        raise RuntimeError("V4-C control row count differs from frozen V3-B reference")
    if not _normalize_identity(control_predictions).equals(_normalize_identity(reference_predictions)):
        raise RuntimeError("V4-C control row identity/order differs from frozen V3-B reference")
    ...
    if not np.allclose(new_score, ref_score, rtol=0.0, atol=CONTROL_SCORE_ATOL, equal_nan=False):
        raise RuntimeError(
            "V4-C exact V3-B score equivalence failed: "
    ...
    for column in METRIC_COLUMNS:
        ...
        if not np.allclose(a, b, rtol=0.0, atol=METRIC_ATOL, equal_nan=False):
            raise RuntimeError(f"V4-C V3-B metric equivalence failed {column}: max={maximum}")
...
def _gate(
    ...
    detail = {
        ...
        "median_top_decile_lift_change": float(
            np.median(
                pd.to_numeric(paired["top_decile_lift_change"], errors="raise").to_numpy(dtype=float)
            )
        ),
    }
    passed = bool(
...
def _top_decile_overlap(
    ...
    for fold in FOLD_NAMES:
        ...
        if not control[identity].equals(candidate[identity]):
            raise RuntimeError(f"V4-C top-decile identity mismatch for {fold}")
        left = assign_within_date_buckets(
            control, score_column="score", buckets=10, output_column="decile"
        )
        right = assign_within_date_buckets(
            candidate, score_column="score", buckets=10, output_column="decile"
        )
        left_keys = set(
            zip(left.loc[left["decile"].eq(10), "date"], left.loc[left["decile"].eq(10), "ticker"], strict=False)
        )
        right_keys = set(
            zip(right.loc[right["decile"].eq(10), "date"], right.loc[right["decile"].eq(10), "ticker"], strict=False)
        )
...
def run_v4c_first_pass(
    *,
    ...
    """Atomic V4-C first pass: exact V3-B control + one dispersion challenger."""
    ...
    control_metrics, control_predictions, model_hashes = _score_candidate(
        table, V4_C_CONTROL, output_dir=control_dir
    )
    elapsed[V4_C_CONTROL] = time.perf_counter() - t0
    ...
    equivalence_path = output_dir / "ranking_v4_c_v3_b_control_equivalence.json"
    equivalence_path.write_text(
        json.dumps(equivalence, indent=2, ensure_ascii=False, default=str), encoding="utf-8"
    )

    challenger_dir = output_dir / "dispersion"
    challenger_dir.mkdir(parents=True, exist_ok=False)
    t0 = time.perf_counter()
    challenger_metrics, challenger_predictions, model_hashes = _score_candidate(
        table, V4_C_CHALLENGER, output_dir=challenger_dir
    )
    elapsed[V4_C_CHALLENGER] = time.perf_counter() - t0
    all_metrics.append(challenger_metrics)
    all_predictions.append(challenger_predictions)
    all_model_hashes.update({f"dispersion/{k}": v for k, v in model_hashes.items()})

    paired, gate_detail, passed = _gate(challenger_metrics, control_metrics)
    paired.insert(0, "candidate", V4_C_CHALLENGER)
    overlap = _top_decile_overlap(control_predictions, challenger_predictions)
    overlap.insert(0, "candidate", V4_C_CHALLENGER)

    metrics_path = output_dir / "ranking_v4_c_f1_f6_metrics.csv"
    predictions_path = output_dir / "ranking_v4_c_f1_f6_predictions.parquet"
    paired_path = output_dir / "ranking_v4_c_f1_f6_paired.csv"
    overlap_path = output_dir / "ranking_v4_c_f1_f6_top_decile_overlap.csv"
    verdict_path = output_dir / "ranking_v4_c_f1_f6_verdict.json"
    runtime_path = output_dir / "ranking_v4_c_f1_f6_runtime.json"
    ...
    verdict = {
        "status": "V4_C_FIRST_PASS_COMPLETE",
        "hypothesis_id": "V4-C-CROSS-SECTIONAL-CONTEXT-V1",
        "control": V4_C_CONTROL,
        "challenger": V4_C_CHALLENGER,
        "challenger_verdict": "PASS" if passed else "FAIL",
        "gate_pass": bool(passed),
        "gate_detail": gate_detail,
        "candidate_ordinals_viewed": [18, 19],
        "within_family_integration_exists": False,
    ...
    runtime = {
        "mode": "atomic_first_pass_control_plus_one_challenger_no_midrun_adaptation",
    ...
    runtime_path.write_text(
        json.dumps(runtime, indent=2, ensure_ascii=False, default=str), encoding="utf-8"
    )
    ...
    summary = {
        ...
        "hypothesis_id": "V4-C-CROSS-SECTIONAL-CONTEXT-V1",
        "candidate_ids": list(V4_C_FIRST_PASS_CANDIDATES),
        "folds": list(FOLD_NAMES),
        ...
        "challenger_result": {
            "verdict": verdict["challenger_verdict"],
            "gate_pass": bool(passed),
            "gate_detail": gate_detail,
            "feature_columns": list(candidate_feature_columns(V4_C_CHALLENGER)),
            "feature_order_sha256": feature_order_sha256(candidate_feature_columns(V4_C_CHALLENGER)),
        },
    ...
    summary_path = output_dir / "ranking_v4_c_f1_f6_summary.json"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run frozen V4-C cross-sectional-context first-pass comparison"
    )
