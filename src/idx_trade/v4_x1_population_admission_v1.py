...
class PopulationScoreGate:
    ...
    def __init__(self, score_module: ModuleType, **kwargs: Any):
        ...
        self.feature_basis_producer = kwargs.pop("feature_basis_producer", None)
        self.kwargs = kwargs
    ...
    def __enter__(self) -> "PopulationScoreGate":
        ...
        def guarded(*args: Any, **kwargs: Any) -> Any:
            admission_kwargs = dict(self.kwargs)
            if self.feature_basis_producer is not None:
                try:
                    feature_basis_result = self.feature_basis_producer(
                        runtime_root=admission_kwargs["runtime_root"],
                        clean_panel=admission_kwargs["clean_panel"],
                        repo_root=admission_kwargs["repo_root"],
                        observed_by=admission_kwargs["observed_by"],
                        trusted_producer_contract=admission_kwargs.get(
                            "trusted_producer_contract"
                        ),
                    )
                except Exception as error:
                    feature_basis_result = {
                        "status": SOURCE_CAPTURE_UNRESOLVED,
                        "reason_codes": [
                            f"FEATURE_BASIS_PRODUCER_ERROR:{type(error).__name__}:{error}"
                        ],
                    }
                admission_kwargs["feature_basis_result"] = feature_basis_result
            admission = build_runtime_population_admission(**admission_kwargs)
