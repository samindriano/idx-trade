...
from .ranking_v4_price_path import (
    MAX_V4_B_HISTORICAL_SIGNAL_INDEX,
...
from .research_v4_price_path import (
...
V4_B_CACHE_STATUS = "RANKING_V4_B_PRICE_PATH_CACHE_FROZEN_PRE_OUTCOME"


def _read_panel_bounded(path: Path, max_date: pd.Timestamp) -> pd.DataFrame:
    if path.suffix.lower() not in {".parquet", ".pq"}:
        raise RuntimeError("V4-B feature preparation requires Parquet signal panel")
    ...
    if frame.empty:
        raise RuntimeError("V4-B bounded signal panel is empty")
    frame = frame.copy()
    frame["date"] = (
    ...
    if (frame["date"] > max_date).any():
        raise RuntimeError("V4-B panel physical read escaped session-1224 boundary")
    return frame
...
def _assert_v3_base_cache(cache_path: Path, manifest_path: Path) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if manifest.get("status") != V3_FINAL_LATE_CACHE_STATUS:
        raise RuntimeError("V4-B V3-B base manifest has unexpected status")
    if bool(manifest.get("post_1224_materialized", True)):
        raise RuntimeError("V4-B V3-B base manifest claims session 1225+ materialization")
    table = pd.read_parquet(cache_path)
    if table.empty:
        raise RuntimeError("V4-B V3-B base cache is empty")
    table = table.copy()
    table["ticker"] = (
    ...
    if missing:
        raise RuntimeError(f"V4-B V3-B base cache missing columns: {sorted(missing)}")
    if table.duplicated(["ticker", "date"]).any():
        raise RuntimeError("V4-B V3-B base cache contains duplicate ticker/date rows")
...
def prepare_v4b_cache(
    *,
    ...
    """Append frozen B1+B2 features to exact V3-B rows without scoring outcomes."""
    ...
    if source_hashes != expected:
        raise RuntimeError(f"V4-B source identity mismatch: expected={expected} actual={source_hashes}")

    base, base_manifest = _assert_v3_base_cache(v3_cache_path, v3_manifest_path)
    sessions = _read_calendar(calendar_path)
    if len(sessions) < MAX_V4_B_HISTORICAL_SIGNAL_INDEX:
        raise RuntimeError("official calendar does not cover V4-B session 1224")
    max_date = pd.Timestamp(sessions[MAX_V4_B_HISTORICAL_SIGNAL_INDEX - 1])
    panel = _read_panel_bounded(panel_path, max_date)
    path = build_price_path_features(
        panel,
        sessions,
        max_signal_session_index=MAX_V4_B_HISTORICAL_SIGNAL_INDEX,
    )
    assert_historical_boundary(path)

    path_keyed = path.set_index(["ticker", "date"])
    keys = pd.MultiIndex.from_frame(base[["ticker", "date"]])
    if not keys.is_unique:
        raise RuntimeError("V4-B base keys are not unique")
    missing_keys = keys.difference(path_keyed.index)
    if len(missing_keys):
        raise RuntimeError(f"V4-B price-path join has {len(missing_keys)} orphan V3-B rows")

    joined = base.copy()
    aligned = path_keyed.reindex(keys)
    for column in V4_B_FEATURE_COLUMNS:
    ...
    if not joined.loc[:, original_columns].equals(base.loc[:, original_columns]):
        raise RuntimeError("V4-B preparation changed existing V3-B cache columns")
    if tuple(V4_B1_MODEL_FEATURE_COLUMNS[: len(V3_B_FEATURE_COLUMNS)]) != tuple(V3_B_FEATURE_COLUMNS):
        raise RuntimeError("V4-B1 does not preserve exact V3-B feature prefix")
    if tuple(V4_B2_MODEL_FEATURE_COLUMNS[: len(V3_B_FEATURE_COLUMNS)]) != tuple(V3_B_FEATURE_COLUMNS):
        raise RuntimeError("V4-B2 does not preserve exact V3-B feature prefix")
    if joined.duplicated(["ticker", "date"]).any():
        raise RuntimeError("V4-B prepared cache contains duplicate ticker/date rows")
    assert_historical_boundary(joined)

    for column in V4_B_FEATURE_COLUMNS:
        values = pd.to_numeric(joined[column], errors="coerce").to_numpy(dtype=float)
        if np.isinf(values).any():
            raise RuntimeError(f"V4-B prepared feature contains infinity: {column}")

    cache_path = output_dir / "ranking_v4_b_price_path_prepared_cache.parquet"
    joined.to_parquet(cache_path, index=False)
    manifest = {
        "status": V4_B_CACHE_STATUS,
        ...
        "v4_b1_feature_columns": list(V4_B1_FEATURE_COLUMNS),
        "v4_b2_feature_columns": list(V4_B2_FEATURE_COLUMNS),
        "v4_b1_model_feature_order_sha256": feature_order_sha256(V4_B1_MODEL_FEATURE_COLUMNS),
        "v4_b2_model_feature_order_sha256": feature_order_sha256(V4_B2_MODEL_FEATURE_COLUMNS),
        "coverage": _coverage(joined),
        "panel_columns_loaded": ["ticker", "date", "high", "low", "close"],
    ...
    manifest_path = output_dir / "ranking_v4_b_price_path_prepared_cache_manifest.json"
