...
V4_X1_DECISION_V2_MINIMAL_PROFILE_V1 = DecisionV2Profile(
    rule_id="V4_X1_DECISION_V2_MINIMAL_V1",
    target_count_max=10,
...
def rank_session_from_v4_x1_verified(verified: VerifiedScoreSession) -> RankSession:
    if not isinstance(verified, VerifiedScoreSession) or verified._verification_token is not _VERIFIED_TOKEN:
        raise DecisionV2Error("DECISION_V2_V4_X1_VERIFIED_SCORE_SESSION_REQUIRED")
    if verified.model_id != EXPECTED_ALPHA_MODEL_ID:
        raise DecisionV2Error("DECISION_V2_V4_X1_MODEL_ID_CHANGED")
    if verified.model_fingerprint != EXPECTED_ALPHA_MODEL_FINGERPRINT:
        raise DecisionV2Error("DECISION_V2_V4_X1_MODEL_FINGERPRINT_CHANGED")
    if not isinstance(verified.scores, pd.DataFrame):
        raise DecisionV2Error("DECISION_V2_V4_X1_SCORE_FRAME_REQUIRED")
    required = {"ticker", "rank_consensus"}
    missing = required - set(verified.scores.columns)
    if missing:
        raise DecisionV2Error(f"DECISION_V2_V4_X1_SCORE_COLUMNS_MISSING:{sorted(missing)}")
    ...
    if frame["rank_consensus"].duplicated().any():
        raise DecisionV2Error("DECISION_V2_V4_X1_DUPLICATE_RANK")
...
def plan_v4_x1_decision_v2_minimal(
    ...
    return plan_decision_v2_minimal(
