...
- requires explicit statement-scope evidence before PIT readiness.
...
A second, more fundamental question must be answered before freezing the parser contract: determine whether scope is truly one value per filing or whether a single filing can legitimately contain both consolidated and separate/company-only statement facts. If mixed scope exists, scope must be represented at the statement/fact level rather than forcing one `statement_scope` value for the entire filing. The current singular adapter field must not become a hidden modeling assumption.

## Authorized next milestone

A separate bounded **statement-scope resolution feasibility** milestone is authorized.

The next task should not rerun the 7,370-row network census and should not derive fundamentals or train models. Reuse the already captured immutable attachments.

The first question is scope granularity:

1. inspect representative filings and establish whether the authoritative filing content expresses one scope per filing or multiple scopes within one filing;
2. if mixed scope exists, design the parser output at statement/fact granularity and leave the existing filing-level `PIT_READY` gate unresolved until the adapter contract is revised;
3. only if one filing can defensibly be assigned a single explicit scope may a filing-level `CONSOLIDATED`/`SEPARATE` resolver be accepted.

Use a representative stratified sample across:
...
The parser must preserve the exact source location/evidence supporting each scope classification and fail closed on conflicting or ambiguous content. No heuristic based only on filenames, issuer type, or endpoint metadata is acceptable.

Only after independent validation of scope semantics and extraction should the existing 6,108 exact joins be reclassified and PIT-ready coverage recomputed. Feature engineering and modeling remain unauthorized until that later gate.
