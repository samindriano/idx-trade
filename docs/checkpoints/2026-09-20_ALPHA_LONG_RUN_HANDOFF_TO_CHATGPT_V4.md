...
The subsequent component-anatomy audit reconstructed the C1/C2/C4 market-side
formula components and matched all finite stored scores exactly: 295,243 for
C1, 310,761 for C2, and 310,323 for C4, each with maximum absolute difference
`0.0`. C1 and C4 rank geometry is dominated by their reversal numerators
(median daily score/component Spearman `-0.9599` and `-0.9519`), while C2 is an
interaction between `ret_5` and abnormal-turnover log rather than a single
