...
| R-20 | Listing/delisting lifecycle admission | The local IDX lifecycle archive is structurally exact but event-level only; six ticker conflicts remain, including one delisting-before-listing row, and no daily PIT membership or issuer/ISIN/publication/vintage/CA chain is available. | Universe masks, survivorship handling, and price-basis joins could be wrong even when raw files hash-match. | `OPEN / BLOCKED` | Keep the surface capability-only; require independently admitted daily membership and identity/timing authority before any feature, repair, or protected read. |
| R-21 | LBRE free-float lineage and PIT admission | The 58,671-file artifact manifest is exact, but 868 normalized rows remain unresolved across multiple-original ambiguity, missing originals, and invalid correction chronology; coverage is monthly issuer-report data. | Free-float inputs or universe masks could encode unresolved revisions, incomplete history, or non-PIT information despite perfect raw hashes. | `OPEN / BLOCKED` | Keep LBRE capability-only; resolve lineage and establish daily population, issuer/ISIN, CA, revision, and public-availability contracts before use. |
| R-22 | Statutory free-float anchor continuity | The 2025-12-31 market anchor has 33 missing explicit-share rows and the 2026-03-31 anchor is percentage-only; embedded LBRE lineage also has 93 excluded and 18 parse-unresolved rows. | Sparse anchors can be mistaken for a continuous share-basis or float history and contaminate universe/CA interpretation. | `OPEN / BLOCKED` | Keep as anchors only; require continuous exact-share, identity, CA, revision, and PIT authority before any feature or mask use. |
| R-23 | HSC ownership event-ledger admission | The 137-artifact ledger is structurally exact with 59 events and 55 effective cutoff tickers, but it is event-only and does not prove population completeness, issuer/ISIN continuity, CA linkage, revision completeness, or public availability. | Event-derived ownership or concentration features could silently treat an incomplete event set as a daily PIT panel or leak post-publication knowledge. | `OPEN / BLOCKED` | Keep HSC capability-only; require population-wide daily coverage, event-to-issuer identity, publication/available-at, and revision/vintage authority before any feature or mask use. |
| R-24 | Broker/margin snapshot semantics and coverage | The 73-file snapshot is structurally exact, but 106/326 eligible tickers are absent from Margin Summary, all-six metric equality is 0/220, and the UI/source identifies IDX Reporting (Regular and Cash), not financing accounts. | Treating the category view as financing flow or a complete tradable universe can create false features and selection bias; no publication/knowledge time exists. | `OPEN / BLOCKED` | Keep snapshot-only; require historical series, explicit financing semantics, complete eligible coverage, and PIT/public-availability authority before any feature or mask use. |
...
| Historical foreign-flow PIT admission | Archive/hash/calendar/schema integrity passes, but all sessions have unknown publication time and only a declared T+1 causality rule | `OPEN / BLOCKED` | Keep the archive in H-FLOW capability only; require public-availability, identity/CA, revision, and completeness authority |
