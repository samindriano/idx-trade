...
| Portfolio accounting / NAV / cash | CROSS_COMPONENT_TESTED | Receivable contributes to NAV but not spendable cash; payment-on-decision sizing hash mismatch found; a synthetic IDR125,000 settlement changes cash while a pending replacement remains unchanged. | Settlement-to-replan policy and residual cash taxonomy. | Three timing cases plus explicit pending/settlement policy. |
...
| Historical design archaeology | STRUCTURALLY_AUDITED | Execution history shows zero-lot buy pending, later partial-sell persistence, and replay acceptance of positive buy underfill as membership-complete. | Historical contract asymmetry must be reconciled before remediation semantics are chosen. | Trace any future quantity-obligation design against prior state/replay contracts. |
| Cross-component interfaces | CROSS_COMPONENT_TESTED | CA→sizing→execution hash mismatch; sizing→execution positive partial loss across four trigger classes; repeated partial exit plus CA payment preserves pending state; Decision reversal can remove pending rows without explicit cancellation; identity→replacement mismatch. | Need a single obligation/state graph and settlement/reversal policy. | Multi-session obligation ledger with lifecycle/audit events. |
