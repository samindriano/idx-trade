...
| Portfolio construction / sizing | CROSS_COMPONENT_TESTED | EOD-close sizing can plan 5,000 shares while Open execution fills 2,500. | Positive partial fills are not a sizing-state contract. | Quantity-aware target/residual model. |
...
| Execution / pending / replacement | CROSS_COMPONENT_TESTED | Partial sells persist; alias replacement peer can block a fully completed sell; positive partial buys disappear. | Residual buy and canonical relationship identity. | Unified order-obligation ledger. |
| Transaction-cost assumptions | STRUCTURALLY_AUDITED | Fee/slippage/stamp rules are explicit and allocator-aware. | Positive partial buy under fee/gap can be misclassified as complete. | Cost-aware residual obligation tests. |
| Liquidity / capacity | CROSS_COMPONENT_TESTED | Reference-day capacity and Open allocation interact with sizing; positive partial buy gap reproduced. | Capacity-limited buy residual semantics. | Capacity stress matrix with partial buy/sell symmetry. |
...
| Dividends / CA integration | CROSS_COMPONENT_TESTED | Entitlement/receivable/payment lifecycle and restart tested; projected-vs-raw state incompatibility found. | Noncash CA remains fail-closed/external-blocked. | CA lifecycle into residual-aware execution. |
| Risk / concentration / exposure | STRUCTURALLY_AUDITED | Seat cap, per-name cap, liquidity and concentration audits exist. | Partial positive positions can create silent underexposure; concentration metrics may miss obligation state. | Risk metrics with target-vs-filled quantities. |
...
| Artifact integrity | STRUCTURALLY_AUDITED | Hash chains and exclusive prospective gate outputs tested. | Hashes detect internal tamper, not semantic partial-fill loss. | Bind quantity obligations into artifact schema. |
...
| Restart / reload / idempotency | CROSS_COMPONENT_TESTED | 59 runtime tests and CA chain reload pass; wrong positive partial position reloads consistently. | Idempotency can preserve semantically incomplete state. | Replay with residual order obligations. |
...
| Cross-component interfaces | CROSS_COMPONENT_TESTED | CA→sizing→execution hash mismatch; sizing→execution positive partial loss; identity→replacement mismatch. | Need a single obligation/state graph. | Multi-session residual-aware system harness. |

## Current matrix interpretation

The system is no longer best described as “component tests mostly pass with a few isolated gaps.” The highest-risk pattern is semantic state loss at boundaries: a component can produce an internally valid artifact whose meaning is incomplete for the next component. The most valuable next work is quantity-aware state modeling and a multi-session residual-order harness, not additional alpha feature search.
