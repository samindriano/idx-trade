...
| REM-OBL-002 | Partial SELL/replacement lacks quantity lineage | `IMPLEMENTED_LOCAL` | `IMPLEMENTED_LOCAL / CANONICAL_REPLAY_GATED / PAIR_RETRY_RESTART_TESTED / PERSISTED_CANONICAL_REPLAY_GATED / ACTIVE_REVERSAL_FAIL_CLOSED / EXPLICIT_CLOSE_RESTART_REPLAY_GATED / REPLAY_OPEN` | Paired retry/reversal and explicit cancellation/relinquishment lineage |
...
| REM-EVAL-001 | Execution evidence loses quantity semantics | `EVIDENCE_V2_IMPLEMENTED_LOCAL` | `IMPLEMENTED_LOCAL / INTRINSIC_REPLAY_GATED / CROSS_PARENT_REPLAY_GATES_CHALLENGE_PASS` | Versioned evidence artifact, structural evaluator replay, nested parent hash, cross-parent lineage, and challenge record |
...
| REM-ID-001 | Identity normalization/revision splits | `IDENTITY_CONTRACT_IMPLEMENTED_LOCAL` | `TRANSITION_BINDING_IMPLEMENTED_LOCAL / CANONICAL_REPLAY_GATED / IDENTITY_EVIDENCE_CANONICAL_REPLAY_GATED / HASH_PINNED_CHILD_WIRING_TESTED / AUTHORITY_OPEN` | Alias, same-class revision, hash-pinned child-consumer artifact, and eventual authoritative identity source |
| REM-CAUSE-001 | Exposure/cash cause state collapses | `CAUSE_V1_IMPLEMENTED_LOCAL` | `TRANSITION_BINDING_IMPLEMENTED_LOCAL / CANONICAL_REPLAY_GATED / REPLAY_ROW_GATES_TESTED / CHILD_REPLAY_GATED` | Cause-to-obligation join, retry transition, and child-consumer replay |
| REM-LINEAGE-001 | Config/runner identity absent from artifacts | `LINEAGE_V2_IMPLEMENTED_LOCAL` | `BOUND_REPLAY_TESTED / CHILD_RUNTIME_BINDING_AND_SELECTION_TESTED / TOP_LEVEL_ARTIFACT_REPLAY_GATED` | Config mismatch and unbound prepared-artifact rejection across prepare/execute/replay |
