...
`restart/replay + explicit snapshot version → Decision seat semantics → CA composition`

## Immediate sequence

1. Add restart/replay and fault-injection coverage around obligation snapshots.
2. Add explicit snapshot/state schema versioning while preserving V1 loaders.
3. Complete Decision seat/reversal semantics without retuning Decision rules.
4. Add legacy classification that never fabricates historical quantities.
...
- explicit snapshot schema migration and legacy classification;
