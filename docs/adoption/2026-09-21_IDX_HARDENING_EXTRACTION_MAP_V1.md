...
| adopt: shadow migration rehearsal | synthetic snapshot provenance/activation/reload/replay test and write-failure fence test | adoption remediation; validates the migration API without live artifacts | migration decision must not mutate source snapshot or silently activate runtime | MIGRATION-PROVENANCE-V1, MIGRATION-ACTIVATION-V1, PAPER-STATE-V2 |
