...
| 17 | Design but do not execute controlled canary | Controlled-canary design explicitly records all 14 rows in its required-field table, with real state/production identity/session window marked `UNSET` fail-closed; it remains unexecuted | DESIGN COMPLETE / NO-GO |
