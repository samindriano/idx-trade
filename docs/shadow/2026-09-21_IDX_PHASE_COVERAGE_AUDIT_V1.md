...
| 4 | Apply migration classifier to real copied population | Every located class is classified; no eligible paper-state row exists to migrate; no permissive class invented | BLOCKED BY ABSENT STATE |
...
| 12 | Construct and execute isolated operational shadow root | Dedicated root contains continuation/controller outputs; five-session continuation and weekend/holiday controller no-op completed without side effects | PASS SYNTHETIC / BLOCKED REAL |
