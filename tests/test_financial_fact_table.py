...
def _xlsx_fixture(*, hidden_scope: str | None = None, visible_scope: str = "Entitas grup / Group entity") -> bytes:
    ...
    sheet1 = f'''<worksheet xmlns="{MAIN_NS}"><sheetData>
      <row r="1"><c r="A1" t="inlineStr"><is><t>Rupiah / IDR</t></is></c></row>
...
def _xbrl_fixture(*, context: str = "CurrentYearInstant", plain_label: bool = False, second_value: str | None = None) -> bytes:
    ...
    second = f'<ix:nonFraction name="idx-cor:Assets" contextRef="CurrentYearInstant" unitRef="IDR" scale="0">{second_value}</ix:nonFraction>' if second_value else ""
    html = f'''<html xmlns:ix="http://www.xbrl.org/2013/inlineXBRL"><head>
      <link:schemaRef xmlns:link="http://www.xbrl.org/2003/linkbase" xlink:href="idx-cor-2024.xsd" xmlns:xlink="http://www.w3.org/1999/xlink"/>
      </head><body>{scope_block}
      <ix:nonFraction name="idx-cor:Assets" contextRef="{context}" unitRef="IDR" scale="0">100</ix:nonFraction>{second}
    </body></html>'''
    payload = io.BytesIO()
    with zipfile.ZipFile(payload, "w") as archive:
