...
def test_classifier_does_not_upgrade_contradiction_to_extended_auction():
    rows = []
    for index in range(3):
        for session, count in (("regular", 1), ("extended", 0)):
            rows.append({"ticker": f"T{index}", "date": "2021-07-01", "timeframe": "60", "session": session, "status": "AVAILABLE", "preopen_bar_count": count})
    assert classify_live_probe(pd.DataFrame(rows), {"decision_rules": {"minimum_consistent_pairs_for_confirmed_exclusion": 3}}) == "TV60_OPEN_BOUNDARY_PATTERN_FOUND_MEANING_UNPROVEN"
