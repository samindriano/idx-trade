...
def test_identical_same_date_ohlcv_collapses_to_the_same_frame_after_reversal():
...
def test_single_ohlcv_row_preserves_existing_canonical_values():
    row = {"date": "2025-01-01", "open": 100.0, "high": 105.0, "low": 95.0, "close": 102.0, "volume": 1000.0}
    result = canonicalize_ohlcv(pd.DataFrame([row]), "TEST")
    assert result[["date", "raw_open", "raw_high", "raw_low", "raw_close", "raw_volume"]].to_dict("records") == [
        {
