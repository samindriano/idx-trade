...
| Direct IDX endpoint discovery audit | `ACTIVE` | `Codex/IDX-Direct-Endpoint-Audit` | `data/idx-direct-endpoint-audit-v1` / `097a793` | Bounded read-only audit of `nichsedge/idx-bei` and direct official IDX endpoints for PIT sector, listing history, corporate actions/shares, and financial-report provenance. No model integration, bulk backfill, protected outcomes, or dataset mutation. |
