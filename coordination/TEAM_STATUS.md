...
Last coordinated update: 2026-08-25 19:33 Asia/Jakarta
...
| 100-session prospective alpha evaluation | `ACTIVE` | `research/idx-forward-evaluation-protocol-v1`; `codex/idx-forward-100-evaluator-v1`; `research/idx-v4-x1-prospective-evaluation-protocol-v1`; merged PR #83 (`bd251c1c`); active base lane `ops/v4-x1-prospective-preaccess-readiness-v1` | V4-X1 protocol, deterministic development-safe evaluator, canonical target identity, and the protected-access gate are frozen/integrated. New work is limited to an outcome-blind pre-access readiness/control plane: partial first-100 inventory validation, calendar-only H5/H10 eligibility, safe operational-attestation readiness, code/contract/access-state checks, and future exact preflight-bundle assembly. It must never read target values, protected outcomes, prospective performance, mutate the canonical counter, call providers, or alter model/Decision/Sizing/Execution/runtime/scheduler. Protected outcomes remain sealed. |
...
| E2E Baseline Paper V1 integration | `ACTIVE` | `integration/idx-e2e-baseline-paper-v1`; integrated HEAD `9a2d5f45`; validated Open implementation `a17d580a0dbd89c648215043281b6f995385bec2` | Decision V2 → Sizing V1 and Decision V2 → Execution V1 adapters are accepted. Execution-grade official IDX `OpenPrice` evidence is accepted with `DIRECT_IDX_THEN_ZAPI_RAW_V1`; real fallback smoke passed at 959/959/959 and the hardened verifier accepted it. PR #85 (Open runtime hardening) and PR #86 (outcome-blind evidence-health audit) are merged into the integration lineage. Windows task `IDXTrade-E2E-OfficialOpen` is installed with 09:02–09:22 retries + AtLogOn; the 2026-08-25 holiday headless smoke returned `WEEKEND_NO_SESSION`/`HOLIDAY_NO_SESSION` with exit/result 0 and no provider calls. First genuine weekday same-session capture remains pending. No retroactive capture or protected-outcome access is authorized. |
...
Continue `integration/idx-e2e-baseline-paper-v1` as the single primary E2E lane. PR #85 and the outcome-blind PR #86 health layer are integrated at `9a2d5f45`; observe the first genuine weekday same-session capture operationally, but do not force a holiday/provider run or retroactively fill a session. In parallel, build only the outcome-blind V4-X1 pre-access readiness/control plane on `ops/v4-x1-prospective-preaccess-readiness-v1`; keep the protected-outcome access gate/runner synthetic-only, keep target values sealed, and do not mutate the forward counter or deployed runtime. Do **not** start another Decision or alpha challenger first.