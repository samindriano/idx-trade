...
| V4-3 prefit runtime capture | `BLOCKED` | `Codex/V4-3-Prefit-Runtime` | `research/idx-ranking-v4-3-prefit-runtime-v1` / final `9d3452528acb481dac679d3b2239decd627730e3` | Preflight failed closed: protocol expects SHA `835da855...` for `config/ranking_v4_3_preregistration.json`, while exact HEAD bytes are `3a54dcf...`; config/protocol were not changed. Focused tests `8 passed, 1 failed`; compile and diff-check pass. Capture did not start; no targets/ranks/model/predictions/performance/provider/protected/fresh-forward outcomes accessed. Await ChatGPT review of the stale byte pin. |
