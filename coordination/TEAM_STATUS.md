...
| Research Integrity / Data QA Gate V1 | `ACTIVE` | PR #103 `audit/research-integrity-data-qa-gate-v1@db97ad38`; stacked PR #108 `data/ca-aware-feature-basis-remediation-v1@3c25cdae` | V14 reconciliation remains authoritative: 412 source rows, 387 economic events, 160 resolved, 181 unresolved, 46 non-basis excluded, and 27 proven linkages. The bounded eight-event RIGHTS_HMETD alternate audit remains complete at `data/ca-aware-feature-basis-remediation-v1@4071945d` with controlling V4 manifest `3b5b1035`; its eight IDX HTTP 403 results remain `PROVIDER_DISCOVERY_FAILURE`, not historical-negative evidence. The exactly four-event issuer-official pilot (`SGER`, `PACK`, `ARTO`, `BRMS`) is complete at `data/ca-aware-feature-basis-remediation-v1@3c25cdae`, artifact manifest `81e02d13`; four search-only requests found no issuer-owned transition document, so issuer path is `NOT_RELIABLY_REPEATABLE` within this bounded audit. No completeness or historical-negative authority is claimed. Recommendation is `STOP_RIGHTS_AND_MOVE_TO_NEXT_CA_FAMILY`; do not execute it or start further RIGHTS work without new authorization. Historical negative/as-of/complete-interval authority remains unsupported or unknown; DATA/RESEARCH admission remains blocked. No Phase-E, outcomes, model work, refit/scoring, counter mutation, canonical rewrite, runtime change, production execution, or merge is authorized. |
