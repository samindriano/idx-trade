...
Last coordinated update: 2026-08-25 19:42 Asia/Jakarta
...
| 100-session prospective alpha evaluation | `ACTIVE` | `research/idx-forward-evaluation-protocol-v1`; `codex/idx-forward-100-evaluator-v1`; `research/idx-v4-x1-prospective-evaluation-protocol-v1`; merged PR #83 (`bd251c1c`); active base lane `ops/v4-x1-prospective-preaccess-readiness-v1` @ `502f9efd` | Frozen evaluator/access-gate science remains unchanged. Outcome-blind readiness base now implements 0..100 contiguous inventory structure, protected-reference/access-guard rejection, calendar-only H5/H10 eligibility with mandatory as-of cutoff, frozen model/target identity checks, pre-access component aggregation, contamination precedence, and operator text rendering. Synthetic focused validation: 17 passed. Next work is production adapters/forensics only: map real safe manifests/attestations into this pure core, audit whether a sealed prospective target-materialization/attestation producer already exists, and prepare eventual exact existing-gate preflight assembly without target reads. Never mutate the canonical counter or access protected outcomes. |
...
Continue `integration/idx-e2e-baseline-paper-v1` as the single primary E2E runtime lane and observe the first genuine weekday same-session capture without forcing or retroactively filling evidence. In parallel, continue only `ops/v4-x1-prospective-preaccess-readiness-v1` from its pure outcome-blind base `502f9efd`: next perform production artifact-shape discovery and adapters, explicitly determine whether a sealed prospective target materializer/attestation producer already exists, and connect only safe metadata/hashes into the readiness report. Do not wire to scheduler/runtime, do not add a second counter, do not inspect target values/performance, and do not authorize the existing protected loader.