...
| Clean V2 Open-alpha research pass (V2.1/V2.2) | `PLANNED` | `Codex/Clean-V2-Open-Alpha-Prereg` | `research/idx-v2-open-alpha-prereg-v1` / HEAD `b882fb5` / acceptance `review/idx-v2-open-alpha-prereg-remediation-acceptance-v1@cede829` | `PREREG_REMEDIATION_ACCEPTED_SINGLE_ATOMIC_HISTORICAL_RUN_AUTHORIZED`: exact common support 277,244 / 729, key `e058e5ce`; CONTROL 25, V2.1 28, V2.2 28 hashes independently verified; strict ancestor/session/boolean gates accepted. Exactly one atomic V2 vs V2.1 vs V2.2 historical-development run is authorized with frozen six folds/H10/HGB/evaluator/selection rule. No tuning/rescue, no 31-feature fit, no provider/fresh-forward access, no canonical model/refit/counter change. Stop after immutable result artifacts + tests for independent review. |
...
| Financial PIT direct IDX publication-chain audit | `ACTIVE` | `Codex/Financial-PIT-Direct-Audit` | `data/financial-pit-v1` / reopening from `b446c3c` | Bounded direct `ListedCompany/GetFinancialReport` → official attachment → `ListedCompany/GetAnnouncement` audit for BBCA/AADI/TLKM samples, retention and revision checks only; no bulk acquisition, features, models, or outcomes. |
