...
Last coordinated update: 2026-08-12 22:02 Asia/Jakarta
...
| Expected Payoff V0 feasibility | `ACTIVE` | `ChatGPT/spec + Codex/runtime` | `research/idx-expected-payoff-v0-feasibility` | Freeze and run one historical-development diagnostic on accepted O2 OOF scores only: causal next-session Open entry, fixed H10 Close exit, ATR-normalized + percent payoff, strict provenance/coverage and corporate-action handling. No new payoff model yet, no provider calls, and **no fresh-forward outcome access**. Stop after verdict for independent review. |
