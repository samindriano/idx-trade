...
| V4 CA event-window semantics V1 | `REVIEW` | `Codex/V4-CA-Event-Window-Semantics` | `data/idx-v4-ca-event-window-semantics-v1` / HEAD `8affe6d6024109b438f4d9d4b55e43759c4f8b71` | Validation PASS: `21 passed`, py_compile and diff-check PASS. Stage 1 outcome `V4_CA_EVENT_WINDOW_CONTINUITY_STILL_BLOCKED`: 136 relevant events, 41 exact transitions, 95 schedule-required events / 75 tickers, 0/600 H5/H10/consensus dates, minimum rate `0.7588075881`. Stage 2 did not run: an operator path typo created only an empty `raw` directory under the required output root, then the hardened launcher correctly refused overwrite; no provider call. Per handoff, no forced cleanup/retry and no Stage 3. No V4 target/model/outcome work or contract changes. |
