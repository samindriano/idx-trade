...
| Stockbit intraday forward capture | `ACTIVE` | Codex/Stockbit-Postclose-Fix | `fix/stockbit-intraday-postclose-fix-v1` / based on `data/stockbit-intraday-forward-capture-v1@32f08d0` | Fixing the installed task's pre-close `16:35/17:30 WIB` gate timing: official IDX Stock Summary returned `recordsTotal=0` on 2026-08-13, 2026-08-17..20, so the gate stopped before Stockbit requests. Align the existing task with canonical EOD timing; no second recorder, no model/outcome/counter work. |
