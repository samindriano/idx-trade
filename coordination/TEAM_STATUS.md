...
| Market/index prospective EOD archive extension | `ACTIVE` | `Codex/Forward-EOD-Automation-UI` | `integration/forward-eod-automation-monitoring` / `df8f177` | Integrating reviewed market/index capture with headless catch-up, immutable Open/ OHLCV sidecar coverage, and read-only monitoring UI. Current time is before the 17:00 Asia/Jakarta controlled-capture cutoff; scheduler install remains gated. |
