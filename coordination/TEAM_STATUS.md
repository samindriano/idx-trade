...
| Clean V2 Open-alpha research pass (V2.1/V2.2) | `REVIEW` | `Codex/Clean-V2-Open-Alpha-Prereg` | `research/idx-v2-open-alpha-prereg-v1` / HEAD `b882fb5` / remediation `research/idx-v2-open-alpha-prereg-v1@b882fb5` | `REMEDIATION_COMPLETE_OUTCOME_RUN_STILL_BLOCKED_FOR_REVIEW`: separate CONTROL 25 (`1107bf6a`), V2.1 28 (`9bf62fd9`), and V2.2 28 (`228c3af`) feature identities; combined 31-feature model prohibited; frozen survivor/head-to-head rule; explicit signal/panel session and previous-ancestor PIT/tradability assertions; strict external booleans; common support unchanged at 277,244 rows / 729 tickers, key `e058e5ce`; focused 9 passed; full 48 passed with one pre-existing storage assertion failure. No model fit, outcome/target access, provider call, or canonical counter/model change. Await ChatGPT review. |
