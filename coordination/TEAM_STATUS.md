...
| E2E Paper cloud-first orchestration | `ACTIVE` | `integration/idx-e2e-baseline-paper-v1@043003ee`; launcher `ops/e2e-paper-cloud-launcher-v1@32a63ff4` | #92 is merged into the integration base and #93 is repinned/synced but remains open. Current bounded task: discover and validate the exact private input bundle, run the explicitly activated throwaway ConditionalS3 smoke, then provision immutable `e2e-paper-v1/inputs` only if every gate passes. No provider/R2 production bundle, scheduler, model, outcome, counter, or live E2E cycle is authorized before prerequisites pass. |
