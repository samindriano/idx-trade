...
Last coordinated update: 2026-08-13 14:47 Asia/Jakarta
...
| PIT-safe V2/V3-B/O2 historical reproduction | `REVIEW` | `Codex/PIT-Safe-Lineage-Reproduction` | `codex/pit-safe-v2-v3b-o2-reproduction-research-v1` / HEAD `765bdba` | Exact V2 HGB_XS_MARKET ↔ V3-B control equivalence passes on 144,221 rows with score max-diff 0.0. V3-B late paired gate fails and retains V2. O2 raw diagnostic remains O2_SURVIVOR, but clean-lineage status is O2_DIAGNOSTIC_ORPHANED_PARENT; no downstream automatic propagation, clean model identity, or forward counter. Strict boolean parsing and review-2 tests/full pytest complete. No canonical overwrite, provider call, or protected outcome access. Await ChatGPT final review. |
| Clean V2 Open-alpha research pass (V2.1/V2.2) | `ACTIVE` | `ChatGPT/Open-Alpha-Research` | research-only / no model fit | Outcome-blind design pass: audit prior repo Open experiments, academic rationale, PIT/data coverage, and feature overlap; verify V2.1 exact O2 geometry and select one preregisterable V2.2 Open-derived family. No historical outcome metrics, fitting, tuning, provider capture, or forward outcome access. |
