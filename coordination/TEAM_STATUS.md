...
| Frontend monitoring / capture system | `BLOCKED` | `Codex/Frontend Editorial Tech` — owner keeps this row current | `frontend/editorial-tech-v1` / remote `4580e49` | Remote advanced past `09cccdc` and removed `/compare`, model detail, and the evidence catalog. Local class-filter patch is preserved but cannot be pushed without reconciling the new architecture. |
