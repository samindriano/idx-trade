...
| Financial PIT direct-IDX adapter and coverage census | `REVIEW` | `Codex/Financial-PIT-Adapter-Census` | `data/financial-pit-adapter-census-v1` / final `d1cb537e` (result `5f600f9c`) | Adapter + bounded 2024–2026 census complete. 7,370 expected issuer-periods / 6,580 reports / 6,212 relevant announcement filename matches / 6,108 exact dual-byte hash joins / 0 PIT-ready because statement scope remains unresolved. Blockers: 1,158 missing publication linkages, 74 ambiguous attachments, 2 hash conflicts, 28 HTTP failures, and no universal consolidated/separate parser. Verdict `PARTIAL_SOURCE_USEFUL_PIT_COVERAGE_INCOMPLETE`; raw evidence remains external; no facts/features/models/outcomes. Await ChatGPT review. |
