...
Last coordinated update: 2026-08-12 23:08 Asia/Jakarta
...
| Expected Payoff V1 specification | `ACTIVE` | `ChatGPT/spec` | planned `research/idx-expected-payoff-v1` from V0 remediation anchor `ecec6835...` | Freeze one dedicated historical-development payoff-regression experiment. No model fit until branch-local V1 spec is committed/frozen; no fresh-forward outcome access. |
...
- Expected Payoff V0 is closed with accepted `FEASIBILITY_GO`. Expected Payoff V1 is spec-only until a new branch-local contract is frozen; do not fit/tune a payoff model or access fresh-forward outcomes before that checkpoint.
