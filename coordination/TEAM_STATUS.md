...
Last coordinated update: 2026-08-13 01:40 Asia/Jakarta
...
| Reliability / Uncertainty V1 forward shadow | `ACTIVE` | `ChatGPT/Reliability-V1-Spec` | `research/idx-reliability-uncertainty-v1-forward-shadow` | User authorized next step. Freeze a deterministic `score_margin_reliability` forward-shadow contract only; no reliability ML fit, no trade filtering, no outcome access, no independent counter. Integrate prospectively with existing O2 scoring/archive after spec review. |
