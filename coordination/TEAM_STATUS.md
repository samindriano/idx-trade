...
| E2E Paper cloud-first orchestration | `ACTIVE` | Checkout identity drift is repaired: `IDXTrade-E2E-Paper` and `IDXTrade-E2E-OfficialOpen` now use the clean pinned `32eaaa8e` checkout; the Sunday no-op made no provider/outcome access. | Wait for the next genuine scheduled POST_EOD. |
| Cloudflare production readiness | `WAITING` | Current `origin/main` source has fresh byte-attested dormant active/preparation candidates; live traffic remains Version `322b598b` at 100% observe-only. Seven stale Cloudflare one-shot/controller tasks are disabled, and Windows V1 is the sole active automatic recovery controller. | Preserve observe-only state and wait for the next genuine scheduled canary; do not promote early. |
...
- **Cloudflare:** current `origin/main` source/bundle/config/profile/package identity is freshly attested; preparation returns `CLOUDFLARE_PRODUCTION_PREPARED_NONAUTOMATIC`, while the active candidate is intentionally not promoted. Windows V1 remains the sole active automatic controller and all stale Cloudflare one-shot tasks are disabled.
