...
| Historical OPEN corporate-action scale reconstruction | `ACTIVE` | `Codex/Open-CA-Scale-Reconstruction` | `data/idx-open-ca-scale-reconstruction-v1` / base `617216a` | Bounded offline audit of residual HLC-mismatch rows, prioritizing manual CLEO/SPMA cases. Test only existing independent official corporate-action factors; admit Open only after exact transformed H/L/C equality to canonical values. No market-wide corporate-action acquisition, provider call, canonical panel mutation, model/outcome access, or O2/V4 changes. |
