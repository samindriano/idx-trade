...
| E2E Paper cloud-first orchestration | `ACTIVE` | Checkout identity drift is repaired: `IDXTrade-E2E-Paper` now uses the clean pinned `32eaaa8e` checkout; the Sunday no-op made no provider/outcome access. `IDXTrade-E2E-OfficialOpen` still needs the ACL-protected task update. | Complete the authorized task ACL update, then wait for the next genuine scheduled POST_EOD. |
| Cloudflare production readiness | `BLOCKED` | Current-main source `b3918932` has fresh byte-attested dormant candidates (`a521ac67` active profile and `2e5d9bc3` observe-only preparation); live traffic remains Version `322b598b` at 100% observe-only. Seven stale enabled one-shot/controller tasks cannot be retired because Task Scheduler returns `Access is denied`. | Use an Administrator task-control session to disable the seven stale tasks and update OfficialOpen/ForwardEOD task ACLs; keep Windows authoritative and do not promote. |
...
- **Cloudflare:** current-main `b39189321a4f311b4c3489a3af83f4fb42cd01e2` source/bundle/config/profile/package identity is freshly attested; preparation returns `CLOUDFLARE_PRODUCTION_PREPARED_NONAUTOMATIC`, while the active candidate is intentionally not promoted. Windows V1 remains the automatic controller, but stale enabled one-shot tasks are an ACL blocker.
