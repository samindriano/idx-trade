...
|---|---|---|---|---|
| Repository artifact governance V1 | `ACTIVE` | `Codex/Artifact-Governance` | `codex/artifact-governance-v1` / base `60c21bf` | Read-only inventory and safe Git-vs-external classification for IDX-Trade artifacts. Implementation is limited to a small registry, documentation, and narrowly scoped ignore rules; no moves/deletes, provider calls, model/outcome changes, or runtime changes. |
