...
| Stockbit Stream prospective archive V1 | `ACTIVE` | `Codex/Stockbit-Stream-Prospective-Archive` | `data/stockbit-stream-prospective-archive-v1` / bootstrap from `origin/main` | Outcome-blind prospective Stockbit Stream capture only. Existing Stockbit intraday price and EOD runtimes remain untouched. Audit Zapi `finance:stockbit/stream`, deterministic canonical/PIT-safe universe, quota guard, immutable raw/normalized storage, and cloud scheduler feasibility; no model, outcome, sentiment, V4-X1, O2, counter, or historical backfill. |
