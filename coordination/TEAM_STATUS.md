...
| Frontend monitoring / capture system | `REVIEW` | `Codex/Frontend Editorial Tech` | `codex/frontend-compare-v2` / `808b0a5` | Read-only monitoring now presents the automated session archive and all three monitored lanes: O2, V3-B, and V2. Manual capture/date controls are removed; model detail links remain. Build and `/`, `/monitoring`, `/monitoring/models/{o2,v3,v2}`, `/compare` HTTP 200 pass. Local runtime reports V2 + V3-B artifacts for 2026-08-10; O2 remains awaiting runtime score artifact. |
| Market/index prospective EOD archive extension | `WAITING` | `Codex/Forward-EOD-Automation-UI` | `integration/forward-eod-automation-monitoring` / `970eef8` | Integration pushed: headless catch-up, immutable Open/OHLCV sidecar, and read-only `/monitoring` UI. Existing legacy Open gap is `0/831` + `0/837` locally; full pytest `255 passed, 3 warnings`, Next.js build passed. Jakarta was 13:45, so one controlled post-17:00 capture/recovery and scheduler install remain gated. |
