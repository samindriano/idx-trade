...
| Alpha available-data research | `ACTIVE` | `codex/alpha-available-data-20260919` | Corrected Stage A complete: C1/C2/C4 retained as structural `FUTURE_RESEARCH`, C3 blocked on sparse/partial PIT coverage; earlier non-conformant implementation attempts retained as failures. Historical alpha claims remain blocked by authoritative Data QA admission; no target/outcome opened. Frozen-session robustness, independent mask/hash audit, C3 capability audit, target firewall, and program checkpoint pass structural checks. Next: non-redundant target-free structural lab for Top-K churn/persistence, concentration/liquidity, rolling stability, and expanded orthogonality. Data-admission re-entry requirements are recorded in `docs/checkpoints/2026-09-19_ALPHA_RESEARCH_DATA_ADMISSION_REMEDIATION_V1.md`; specification only, not an admission. No incumbent/main/canonical/capture/cloud/R2/scheduler/counter/protected-data mutation; all derived artifacts stay isolated with provenance. |
