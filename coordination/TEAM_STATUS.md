...
Last coordinated update: 2026-08-13 02:40 Asia/Jakarta
...
| Forward 100-session evaluation protocol | `DONE` | `ChatGPT/Forward-Eval-Protocol` | `research/idx-forward-evaluation-protocol-v1` / frozen protocol `6c05499d01ba644c80f0c6bd6d621aac92ab2813` | `FORWARD_100_SESSION_EVALUATION_PROTOCOL_V1_FROZEN_OUTCOME_BLIND`: first vault evaluates O2 primary then Reliability V1 from one immutable outcome access. O2 keeps 100-session + fixed 50/50 stability semantics; Reliability uses only the frozen raw margin with V0 metric family and preregistered `PASS/INCONCLUSIVE/FAIL`; O2.1 is excluded from the first outcome verdict. No protected outcome accessed. Evaluator may be implemented/tested only on non-protected fixtures before a final 100/100 pre-vault review. |
...
| O2.1 sealed shadow diagnostic | `REVIEW` | `ChatGPT independent review + Codex/O2.1-sealed-shadow` | `integration/o2-1-sealed-shadow-v1` / review `5fcad99e992e0098112338b73ba756bcbdcdb478` / authorization `051c6da2e0170c84de4c53e515a41887f4be9e35` | `O2_1_SEALED_SHADOW_REVIEW_CHANGES_REQUIRED`: model/encoding/fingerprints and subordinate no-counter semantics are consistent, but existing sidecar reuse must revalidate source OHLCV/snapshot/model pins and protected flags; `shadow_status()` must validate rather than hard-code safety; exact O2 coverage resolution should fail closed on ambiguity. Preserve the 2026-08-12 shadow artifact SHA `20925d72546f35ccce3a355fdc02d31789c90d20437cffd1db6068481ddd2c34`; no recomputation or outcome access. |
...
| Broker / Margin source audit V0 | `REVIEW` | `ChatGPT/Broker-Margin-Source-Audit` | `data/broker-margin-source-audit-v0` / deep audit `7141650` | Deep public-source audit downgrades the margin-flow interpretation: evidence currently favors a periodic eligibility/Stock Summary view, but semantics are not certified. **Do not interpret as actual margin usage or derive margin-share/leverage/crowding features.** Next gate is only a minimal local H1-vs-H2 parity test against same-date All Stock + official eligible list; historical PIT remains unresolved. |
