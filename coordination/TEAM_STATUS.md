...
| V4 CA blocker attribution V1 | `REVIEW` | `Codex/V4-CA-Blocker-Attribution` | `data/idx-v4-ca-blocker-attribution-v1` / `07b2a18eb0e717bcf83fd545351c2fdb3b56a0c4` | Validation fail-closed: `3 passed, 1 failed` in `tests/test_v4_ca_blocker_attribution.py`, specifically `test_consensus_uses_h5_h10_intersection`; `per_date_metrics()` raised `SCENARIO_PER_DATE_COUNT_CHANGED` on the small fixture. No `py_compile`, ledger verification, or attribution run after failure. No patch/provider/semantic/model/target/outcome work. Requires ChatGPT review before any retry. |
