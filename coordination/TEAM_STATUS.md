...
| Expected Payoff V1 | `ACTIVE` | `ChatGPT/spec + Codex/runtime` | `research/idx-expected-payoff-v1` / frozen spec `a5f2ae154505ecc2c3a182711c0900afe38d1759` | `EXPECTED_PAYOFF_V1_HISTORICAL_EXPERIMENT_AUTHORIZED_FROZEN`: run exactly one HGB mean-payoff regressor on the frozen 36 O2 features and exact V0 payoff contract/six folds. No candidate search, no fresh-forward outcome access; stop after verdict for independent review. |
...
- Expected Payoff V0 is closed with accepted `FEASIBILITY_GO`. Expected Payoff V1 is now frozen at `a5f2ae15...`; only that exact one-candidate historical experiment is authorized. No payoff candidate search, post-result rescue, or fresh-forward outcome access.
