...
Last coordinated update: 2026-08-24 22:23 Asia/Jakarta
...
| 100-session prospective alpha evaluation | `ACTIVE` | `research/idx-forward-evaluation-protocol-v1`; `codex/idx-forward-100-evaluator-v1`; `research/idx-v4-x1-prospective-evaluation-protocol-v1` | V4-X1 prospective protocol is frozen outcome-blind. Implement deterministic evaluator on the V4-X1 protocol branch using synthetic/non-prospective fixtures only; do not inspect protected outcomes or auto-unlock the vault. The older O2/Reliability protocol/evaluator remain precedent, not controlling V4-X1 science. |
...
Continue `integration/idx-e2e-baseline-paper-v1` as the single primary E2E lane. The official execution-grade `OpenPrice` transport/verifier/scheduler gate is accepted and deployed; observe the first weekday same-session capture operationally, but do not block development on that observation. In parallel, the V4-X1 prospective evaluation lane may implement and test the preregistered deterministic evaluator strictly on synthetic/non-prospective fixtures, with protected outcomes remaining sealed. Do **not** start another Decision or alpha challenger first.
