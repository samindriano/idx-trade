...
| V4 KSEI coverage-gap remediation V1 | `REVIEW` | `Codex/V4-KSEI-Coverage-Gap` | `data/idx-v4-ksei-coverage-gap-remediation-v1` / `f3a2af51ec925b75498f798f3f1f1bbd72a5f35d` | Preflight stopped fail-closed: focused pytest `7 passed`, `py_compile` PASS, diff-check PASS; `preflight_v4_ksei_coverage_gap_runtime.py` failed before any network/provider call with `ModuleNotFoundError: No module named 'idx_trade.v4_ksei_coverage_gap'`. No immutable-input verification, targeted 43-ticker run, continuity replay, patch, or provider work performed. Requires review before retry. |
