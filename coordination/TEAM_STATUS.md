...
| V4 KSEI coverage-gap remediation V1 | `ACTIVE` | `Codex/V4-KSEI-Coverage-Gap` | `data/idx-v4-ksei-coverage-gap-remediation-v1` / `15dfe535f091b23ad510100a7002830e982415e2` | Import-path retry authorized: no source/config patch; first verify `PYTHONPATH=src`, rerun the unchanged 7-test/compile/diff/preflight gate with zero network calls, then proceed to exactly one targeted 43-ticker KSEI run and one continuity replay only if all preflight and immutable-input gates pass. |
