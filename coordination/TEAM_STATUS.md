...
| V4-X clean-data consolidation V1 | `WAITING` | `ChatGPT/V4-X-Clean-Data-Consolidation` | `data/v4-x-clean-data-consolidation-v1` / `256476d6353a6e8c0d0d6f3727560e8d1384869a` | Stage-A completed with `STAGE_A_CONSOLIDATION_MATERIALIZED_WAITING_FOR_IDENTITY_ADJUDICATION`: exact parent population `981,940` rows / `945` tickers; HLC repairs `1,657` rows / `12` tickers; Open repairs `1,655` (`1,216` official IDX + `439` CA-factor fallback); `2` Open candidates remain fail-closed. Volume and Regular-Market Value parity PASS; identity/universe unchanged. External manifest `eaeabad3c2050142d973d3f8ec350934b995b4e890ea4a12588304d325073969`. Focused `8 passed`. No provider/model/score/target/outcome/counter/refit/V4-X2 work. Waiting for separately owned PIT Security Identity / Listing-Domain adjudication before Stage B. |
...
| PIT Security Identity / Listing-Domain V1 adversarial audit | `ACTIVE` | `Codex/PIT-Security-Identity-Audit` | `audit/pit-security-identity-stage-c-v1` / `0e778d2311eba01e66966f3440262d9ea50cc8e2` | Stage C — exact V4-X H5/H10 final-training support intersection. Authorized state-only projection: ticker/date/target_state_h5/target_state_h10 plus frozen per-date eligibility. Numeric target ranks, returns, labels, predictions, models, providers, protected/fresh-forward outcomes, and counters remain locked. |
