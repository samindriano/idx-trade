...
| Research Integrity / Data QA Gate V1 | `ACTIVE` | PR #103 `audit/research-integrity-data-qa-gate-v1@4a2cfbe3` | Fail-closed Data Admission → Research Admission → Model Promotion framework, reusable invariants, QA orchestration mode, standardized gate CLI, and incident ledger are implemented. Next: exact-head local pytest/CLI validation, then apply the gate to historical CA/price-basis integrity with independent artifact-level orchestra. Connector-authored commits did not produce a GitHub Actions run. No V4-X1 refit, outcome access, counter mutation, capture/runtime change, or science rescue is authorized in this lane. |
...
| E2E Paper cloud-first orchestration | `ACTIVE` | main workflow `.github/workflows/e2e-paper-cloud-orchestration.yml` at `77acb20c`; implementation pin `6e1bf4a1e47a2abff365b35c19687444cf3f0596`; input bridge `ops/e2e-paper-cloud-input-provisioning-v1@13cd07af` | Existing E2E schedule is enabled with PREOPEN retries at 09:03/09:13/09:22 and POST_EOD retries at 18:35/19:05/19:35 Asia/Jakarta. The accepted implementation pin is now active in the workflow; GitHub secret wiring remains repository-managed. No new production provider/R2 bundle or protected outcome was accessed; the first genuine 2026-08-27 scheduled run is the operational proof. |
| Stockbit Intraday cloud migration | `ACTIVE` | main workflow `.github/workflows/stockbit-intraday-cloud-production.yml` at `77acb20c`; merged PR #95 implementation plus PR #105 production schedule | PR #95, PR #104, and PR #105 are merged. Isolated R2 smoke `32992195295` and read-only E2E bridge preflight `32992458786` passed. Production schedule is enabled at 18:30/19:30/20:30 Asia/Jakarta; the Windows fallback remains available and no cutover/retirement is claimed. First genuine post-EOD production proof is still pending. |
...
5. **Stockbit Intraday Capture** — post-close intraday capture/reconstruction; the main GitHub/R2 workflow is scheduled for 18:30/19:30/20:30 Asia/Jakarta, while the Windows runtime remains the fallback until genuine cloud proof and single-writer review.
...
| Stockbit Intraday Capture | `ACTIVE` | main workflow `.github/workflows/stockbit-intraday-cloud-production.yml` at `77acb20c`; Windows fallback `fix/stockbit-intraday-postclose-fix-v1` | Cloud schedule is enabled for 18:30/19:30/20:30 Asia/Jakarta after merged implementation, isolated R2 smoke, and read-only bridge preflight. The Windows fallback remains retained; first genuine production post-EOD proof and single-writer cutover decision are pending. |
...
For E2E cloud-first: observe the first eligible scheduled PREOPEN/POST_EOD cycle on the repinned main workflow and verify the resulting cloud evidence. Keep Windows fallbacks until that proof passes. Do not backfill missed sessions.

For Stockbit Intraday: observe the first eligible production post-EOD cloud cycle after the merged schedule, while retaining the Windows 18:30/19:30/20:30 fallback. Before claiming cutover, define and verify a single-writer operating plan so the existing Windows tasks and cloud workflow cannot both issue provider calls for the same session. Do not backfill missed sessions.
