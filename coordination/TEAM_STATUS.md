...
|---|---|---|---|---|
| Repository Hygiene V1 branch inventory dry-run audit | `ACTIVE` | `Codex/Repository-Hygiene-V1` | `codex/repository-hygiene-v1` / base `origin/main@5c98d8a674ddf6f2da24e7d52e9a308af4c88079` | Issue #30 policy audit only: full remote-branch fetch/prune, ancestry/reachability and documentation-reference inventory. No branch deletion, tag creation, PR closure, history rewrite, force-push, or scientific-lane changes. GitHub PR/Issue metadata requires a later authenticated lookup. |
...
| Financial PIT Alpha V1 preregistration + support census | `REVIEW` | `Codex/Financial-PIT-Alpha` | `research/idx-financial-pit-alpha-v1` / final `507aaf8bca3286996eb30f3f8e7ea161d8892cc1` / frozen contract external `D:\Documents\Project\idx-financial-pit-alpha-20260815-v1-financial-era-contract` / base clean V2 `research/idx-v2-open-alpha-prereg-v1` | `FINANCIAL_PIT_ALPHA_V1_FINANCIAL_ERA_RUN_BLOCKED_ENGINEERING_REAUTH_REQUIRED`: F4/F5/F6 contract preflight passed, but the one authorized historical run stopped after fitting 9 models at an engineering identity-check bug (`KeyError` from tuple DataFrame column selection). No metrics, paired deltas, gate, or verdict are valid. Partial 9-model artifacts preserved externally; corrected runner and checkpoint pushed. Fresh-forward/O2/protected outcomes untouched. New authorization is required before rerun. Full pytest `61 passed, 1 failed` on unrelated storage expectation; focused tests pass. |
