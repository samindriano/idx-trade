...
| Capture/runtime repository hygiene V3 | `REVIEW` | `chore/capture-runtime-hygiene-v3` | Registry defines exactly five capture families and audits stale capture branches before deletion. Ten merged/contained heads are branch-ref redundant; four graph-unique but superseded heads now have temporary exact-head forensic refs. Market/Index EOD audit is closed in favor of the hardened accepted E2E implementation. Open Stockbit red-team PR #36 remains protected. No runtime/scheduler deletion in this lane. |
...
For capture hygiene: merge `CAPTURE_RUNTIME_REGISTRY_V1`; keep PR #36 untouched while open; convert the four temporary exact-head forensic refs (Forward Open scaffold, Stockbit V1 base, Stockbit observable smoke, Market/Index EOD) to permanent archive tags using an API/local Git surface that supports tag creation/deletion; then atomically remove only branches certified `SAFE_TO_DELETE_BRANCH` or `ARCHIVE_EXACT_HEAD_THEN_DELETE`, remove temporary archive branches, and verify default-branch/pinned-SHA workflows afterward.
