...
Last coordinated update: 2026-08-12 22:12 Asia/Jakarta
...
| Expected Payoff V0 feasibility | `ACTIVE` | `ChatGPT/spec + Codex/runtime` | `research/idx-expected-payoff-v0-feasibility` / frozen spec `024c91fcd9c105744b575d5584160b077c848e3e` | Frozen one-shot historical-development diagnostic on accepted O2 OOF scores only: next-session Open entry, H10 Close exit, gross ATR-normalized + percent price payoff, strict provenance/coverage and fail-closed price-scale corporate-action handling. Implement/run exactly once; no payoff model, provider calls, or fresh-forward outcome access; then checkpoint and STOP for review. |
...
- Expected Payoff V0 is frozen at `024c91f...` as a bounded historical-development feasibility diagnostic only. Do not fit a payoff model, change the O2 ranker, access fresh-forward outcomes, or tune entry/exit/horizon/normalization/metrics/gates after seeing the V0 result.
