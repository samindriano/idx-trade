...
| Research Integrity / Data QA Gate V1 | `ACTIVE` | `data/ca-aware-feature-basis-remediation-v1@b5077eeb`; PR #108 | INC-001/V1.1 remains outcome-blind and fail-closed. Narrow RIGHTS_HMETD archive-routing audit is active for exactly SAME/SGER/PACK, using retained KSEI evidence first and at most one planned request per new month key. No remaining RIGHTS acquisition, MMIX retry, other CA family, Phase-E, outcomes, model/refit/scoring, counter mutation, canonical rewrite, production execution, or merge is authorized. |
