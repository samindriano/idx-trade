...
| V4-3 prefit runtime capture | `BLOCKED` | `Codex/V4-3-Prefit-Runtime` | `research/idx-ranking-v4-3-prefit-runtime-v1` / final `fc7ad38fbae2c418ff025465fb367e11d3f5ab5e` | Preflight retry failed closed: canonical `HEAD:config/ranking_v4_3_preregistration.json` is `3a54dcf...`, but frozen protocol expects `835da855...`. Focused tests `9 passed, 1 failed`; compile and diff-check pass. Capture did not start; no targets/ranks/model/predictions/performance/provider/protected/fresh-forward outcomes accessed. Await ChatGPT review of the stale canonical SHA pin. |
