...
Last coordinated update: 2026-08-13 15:49 Asia/Jakarta
...
| Clean V2 Open-alpha research pass (V2.1/V2.2) | `REVIEW` | `Codex/Clean-V2-Open-Alpha-Prereg` | `research/idx-v2-open-alpha-prereg-v1` / HEAD `24311e9d27fb408d5cb8013065f72e81f0e0a59a` | Frozen preregistration and outcome-blind common-support/cache audit complete: 277,244 common-support rows / 729 tickers, key SHA `e058e5ce`, focused tests 5 passed, full pytest 44 passed with one pre-existing storage assertion failure. No model fit/score, target load, provider call, forward outcome access, canonical model/counter change, or O1/O2 rescue. Await ChatGPT independent review. |
...
| Direct IDX endpoint discovery audit | `REVIEW` | `Codex/IDX-Direct-Endpoint-Audit` | `data/idx-direct-endpoint-audit-v1` / `6cf00bc` | Final bounded discovery: 14/14 HTTP 200 for eight `GetIssuedHistory` categories and six `GetFinancialReport` probes. Issued history is useful as an official event/share-count candidate ledger; financial reports are useful filing metadata. Neither is standalone PIT-ready for effective/publication timing; no bulk acquisition or integration. External manifest SHA `9c1c73d9...8bf69`. |
