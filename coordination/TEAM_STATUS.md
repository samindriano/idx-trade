...
|---|---|---|---|---|
| Repository artifact governance V1 | `ACTIVE` | `Codex/Artifact-Governance` | `codex/artifact-governance-v1` / base `e2b7b35` | Performing the authorized actual promotion of only accepted small canonical/summary/manifest artifacts. Raw providers, full panels, attachments, binaries, runtime, credentials, and outcomes remain external. Candidate provenance, SHA, duplicate/stale status, and path-scrubbing are being verified before copy. |
...
| V4 target-support census | `ACTIVE` | `Codex/V4-Target-Support-Census` | `research/idx-v4-target-support-census-v1` / parent `research/idx-ranking-v4-2-evaluation-contract-v1` | Outcome-blind per-date support census for Open(t+1), H5/H10 Close, both-target observability, corporate-action continuity, and mechanism/tradability states using accepted canonical artifacts plus the verified 2,184-row Open overlay. No labels, IC/performance, model fit, provider/CA acquisition, or evaluator-contract changes. |
