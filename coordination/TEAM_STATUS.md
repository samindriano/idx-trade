...
|---|---|---|---|---|
| Repository artifact governance V1 | `REVIEW` | `Codex/Artifact-Governance` | `codex/artifact-governance-v1` / final `e2b7b35` | Inventory and safe Git-vs-external registry complete. Snapshot: 48 origin/main-tracked files, 122 targeted external roots / 107,691 files / about 17.73 GB. Added `artifacts/` registry/schema/drop-zone docs, inventory CSV/MD, and narrow ignore rules; focused tests `2 passed`; full suite `41 passed, 1 pre-existing unrelated storage failure`; no moves/deletes, provider calls, model/outcome changes, or runtime changes. Await ChatGPT review. |
...
| Historical OPEN corporate-action scale reconstruction | `REVIEW` | `Codex/Open-CA-Scale-Reconstruction` | `data/idx-open-ca-scale-reconstruction-v1` / result `6e69ac3` | `HISTORICAL_OPEN_CA_SCALE_RECONSTRUCTION_PARTIAL_RECOVERY`: offline audit of all 49,476 residual Open rows and 32,103 HLC-mismatch rows using existing official IDX factors; 2,184 exact transformed H/L/C rows recovered (including 874 CLEO rows), 47,292 remain unresolved. Audit-overlay coverage is 934,648/981,940 = 95.183820%; 249/1,260 dates are below 90%. No new CA acquisition, provider call, canonical panel mutation, model/outcome access, or O2/V4 changes. See `docs/checkpoints/2026-08-17_HISTORICAL_OPEN_CA_SCALE_RECONSTRUCTION_V1.md`. |
