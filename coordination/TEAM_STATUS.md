...
|---|---|---|---|---|
| Repository Hygiene V1 branch inventory dry-run audit | `REVIEW` | `Codex/Repository-Hygiene-V1` | `codex/repository-hygiene-v1` / final `31cf7f8` | Batch 2 complete: PR #20 closed stale/superseded at unchanged archived HEAD `25eaa67`; 30 exact archive-tagged branches deleted; 30/30 annotated tags remain SHA-verified; action-time branch count reconciled `160 -> 130`, then one unrelated branch appeared (final observed count `131`). Two non-target heads advanced externally and remain untouched. No KEEP/ABANDONED/NEEDS_MANUAL_REVIEW branch was deleted; no other PR, main history, runtime, O2, model, or scientific files touched. See `docs/repository_hygiene/POST_ACTION_REPORT_V2.md`. |
...
| TradingView historical price-path V2.1 remediation | `ACTIVE` | `Codex/TradingView-Historical-Price-Path-V2.1-Remediation` | `data/tradingview-historical-price-path-v2-1-remediation` / base `f6716bb861eee396a8b57c42b207af31b4565db7`; prior remediation `97124d017de9533e1c84d7f84eab4b22edbfbda4`; activity `c943a76fd56872d981a87519c2eb7072c413322c` | Correctness and acquisition-engineering remediation only: offline symbol ceiling, historical identity mapping, session/boundary/ACTIVE-only contracts, official-session-index CA quarantine, fidelity/NO_TRADE diagnostics, serialized request schema, streaming/resume design, and at most five frozen depth-preflight calls. Failed V2 verdict remains unchanged; no 978-ticker rerun, model, panel mutation, Path Risk, O2, outcomes, or provider search. |
...
| Financial Representation V2 structural audit | `ACTIVE` | `Codex/Financial-Representation-V2` | `research/idx-financial-representation-v2` / base `1a9bf7267728d9beec2a975ac4b4e931d0be16d0` | Outcome-blind structural audit only over the accepted Financial PIT panel and clean-V2 identity/calendar support. One same-reporting-period bundle per ticker at the 18:00 WIB cutoff; no feature fallback across bundles; candidate set is CORE3 plus `yoy_revenue` and `yoy_total_assets` only. No labels, predictions, performance artifacts, model fit/score, O2, fresh-forward, or protected-outcome access. |
