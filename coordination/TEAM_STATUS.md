...
| Stockbit Stream prospective archive V1 | `REVIEW` | `Codex/Stockbit-Stream-Prospective-Archive` | `data/stockbit-stream-prospective-archive-v1` / `4a20f2dd` | Existing cloud Stockbit Stream capture now schedules `pre_open`, `midday`, and `after_close` every calendar day, including weekends and IDX holidays. Exchange-session identity remains separate from calendar capture date. Local Stockbit intraday/EOD runtimes, models, outcomes, counters, and historical backfill remain untouched. Focused stream tests pass; no provider call or real workflow run in this change. |
