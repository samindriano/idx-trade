...
| Financial PIT Alpha V1 preregistration + support census | `ACTIVE` | `Codex/Financial-PIT-Alpha` | `research/idx-financial-pit-alpha-v1` / remediation from `13ad4b999c55af91faa2411e28d29303431a6add` / base clean V2 `research/idx-v2-open-alpha-prereg-v1` | `FINANCIAL_PIT_ALPHA_V1_SUPPORT_CENSUS_CONDITIONALLY_ACCEPTED_CUTOFF_AND_RUN_SPEC_REWORK_REQUIRED`: outcome-blind remediation only. Rework the operational cutoff to `18:00:00 Asia/Jakarta` on session t, rerun support, freeze 52 raw Financial slots, audit inherited fold support, and freeze candidate feature-order/survivor contracts before any metrics. No Financial source/materialization edits, labels, outcomes, scores, fitting, or V3-B/O2 lineage reuse. |
