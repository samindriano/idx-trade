...
| Stockbit intraday forward capture | `REVIEW` | Codex/Stockbit-Postclose-Fix | `fix/stockbit-intraday-postclose-fix-v1` / final `88a12dae510062113fe08eeac5bbd30a9253bcb7` | `POSTCLOSE_SCHEDULER_REMEDIATION_INSTALLED_NO_CAPTURE_BEFORE_CUTOFF`: root cause was official IDX Stock Summary `recordsTotal=0` at the old `16:35/17:30 WIB` gate on 2026-08-13 and 2026-08-17..20. Existing task re-registered with EOD-aligned `18:30/19:30/20:30 WIB`, `StartWhenAvailable`, `WakeToRun`, and `IgnoreNew`; next run 2026-08-24 18:30 WIB. Focused `17 passed`, full pytest PASS, no provider call during remediation. Missed today-only sessions are not backfilled. |
| Stockbit Stream prospective archive V1 | `REVIEW` | `Codex/Stockbit-Stream-Prospective-Archive` | `data/stockbit-stream-prospective-archive-v1` / daily-calendar scheduling remediation pending final commit | Existing cloud Stockbit Stream capture now schedules `pre_open`, `midday`, and `after_close` every calendar day, including weekends and IDX holidays. Exchange-session identity remains separate from calendar capture date. Local Stockbit intraday/EOD runtimes, models, outcomes, counters, and historical backfill remain untouched. Focused stream tests pass; no provider call or real workflow run in this change. |
