...
| Research Integrity / Data QA Gate V1 | `ACTIVE` | PR #103 `audit/research-integrity-data-qa-gate-v1@a1096aa1`; stacked PR #108 `data/ca-aware-feature-basis-remediation-v1@6875cef4` | INC-001 controlling reconciliation remains V16 at `D:\Documents\Project\idx-ca-economic-event-reconciliation-20260831-v16-composite-policy`, manifest `3ff25d77dd1d2d85d1ff7526fcef90525dfe60afed026f4c2738b8bfb2a57030`: 412 source rows / 387 economic events / 163 exact-resolved / 178 unresolved / 46 proven non-basis / 27 proven linkages. Closure feasibility remains `KNOWN_EVENT_REMEDIATION=MATERIAL_WORK_COMPLETE`, `KNOWN_EVENT_LONG_TAIL=PARKED_FAIL_CLOSED`, `POPULATION_COMPLETENESS=UNKNOWN`, `HISTORICAL_ASOF_AUTHORITY=UNKNOWN`, `INC001_INCIDENT_CLOSURE=BLOCKED_ON_POPULATION_AUTHORITY`; Data/Research admission and historical application remain blocked. Population-authority feasibility verdict remains `EXISTING_OFFICIAL_SOURCES_CANNOT_CERTIFY_POPULATION_COMPLETENESS`. User-authorized Tier-A ZAPI router work is now bounded and active only to recover event-specific official documents for already-known residual split/reverse events; ZAPI is discovery/router evidence, **not** transition/no-event/population authority. Initial controls exposed RIGHTS revision/semantic conflict, while targeted split router run `33357587316` found DIVA, BBRM, BYAN, SKRN, SMDR, BMRI, TMAS, TUGU. Next action: bind only those router hits to first-party documents and accept an exact transition only when the document explicitly proves the regular-market first-new-basis boundary with immutable provenance. Do not reopen other 178 events, infer dates, claim negative authority, access outcomes, refit/score V4-X1, mutate counters, rewrite canonical data, change production runtime, backfill, deploy, or merge. |
