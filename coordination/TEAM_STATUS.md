...
| Research Integrity / Data QA Gate V1 | `ACTIVE` | PR #103 `audit/research-integrity-data-qa-gate-v1@db97ad38`; stacked PR #108 `data/ca-aware-feature-basis-remediation-v1@5f958319` | R3 adversarial correction is active. Reconcile the exact cross-sectional application population, observed-row backward dependency closure, old-vs-current support lineage, and KSEI coverage against all three populations. Preserve R2 findings: exact final-fit support is 629 tickers versus the 610-ticker KSEI census; KSEI lacks per-session as-of/no-event attestation; and structural family coverage/transition taxonomy remains partial or contradictory, including MERGER/VOLUNTARY_CONVERSION. Phase-E historical recompute remains blocked. No V4-X1 refit, outcome access, counter mutation, capture/runtime change, provider acquisition, historical feature recomputation, or science rescue is authorized. |
