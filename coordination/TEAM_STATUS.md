...
Last coordinated update: 2026-08-13 16:48 Asia/Jakarta
...
| Investing historical intraday depth audit | `REVIEW` | `Codex/Investing-Intraday-Depth-Audit` | `data/investing-intraday-depth-audit-v1` / content commit `74e4587`; review checkpoint pending push | `DEPTH_JUSTIFIES_PREREGISTERED_SECONDARY_INTRADAY_LANE_NOT_BULK_READY`: 3,685/3,685 1-hour probes completed; 2026 available 671/737, 2018 available 326/737, final provider errors 0; 20-ticker fidelity found HLC exact 190/256 and OHLC exact 136/224 comparable rows with CA-like scale anomalies retained fail-closed. Stop for ChatGPT review; no panel/model/backfill work. |
...
| Financial PIT direct-IDX adapter and coverage census | `ACTIVE` | `Codex/Financial-PIT-Adapter-Census` | `data/financial-pit-adapter-census-v1` / from accepted `data/financial-pit-v1@25eaa67` | Implement fail-closed report→announcement→exact attachment/hash→Asia/Jakarta publication adapter, then run bounded 2024–2026 issuer-period readiness census only; preserve raw evidence outside Git and do not build features/models/outcomes. |
