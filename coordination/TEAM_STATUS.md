...
| Frontend monitoring / capture system | `ACTIVE` | `Codex/Forward-EOD-Automation-UI` | `integration/forward-eod-automation-monitoring` / `970eef8` | Fixing the model-evidence selector so the existing O1 and O2 entries remain reachable; UI-only change, no runtime/data/model artifact changes. |
