...
| Clean V2 Open-alpha research pass (V2.1/V2.2) | `ACTIVE` | `Codex/Clean-V2-Open-Alpha-Prereg` | `research/idx-v2-open-alpha-prereg-v1` / HEAD `b882fb5` / acceptance `review/idx-v2-open-alpha-prereg-remediation-acceptance-v1@cede829` / atomic run in progress | `ATOMIC_HISTORICAL_RUN_IN_PROGRESS`: authorized exact common support 277,244 / 729, key `e058e5ce`; CONTROL 25, V2.1 28, V2.2 28 hashes independently verified; strict ancestor/session/boolean gates accepted. Running exactly one V2 vs V2.1 vs V2.2 historical-development comparison with frozen six folds/H10/HGB/evaluator/selection rule. No tuning/rescue, no 31-feature fit, no provider/fresh-forward access, no canonical model/refit/counter change. Stop after immutable result artifacts + tests for independent review. |
