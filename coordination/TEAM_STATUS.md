...
| Foreign-flow transition alpha challenger | `ACTIVE — corrected shadow, partial run binding, admission blocked` | `codex/alpha-challenger-pit-safe-20260919` | Corrected direction audit covers 15 clean prospective score sessions; transition is available on 5/15, with mean corrected top-30 churn `15.3333%` and max `20%`; no performance claim. The earlier `100%` overlap / `0%` churn readout was invalidated because it mixed rank directions. EOD/V4 run IDs still bind snapshot/OHLCV but not raw Stock Summary SHA/attempt, and security-master as-of remains unattested. |
...
| Price/trend alpha challenger | `ACTIVE — structural shadow only, admission blocked` | `codex/alpha-challenger-pit-safe-20260919` | Fixed outcome-blind 90/10 overlay preregistered; 15 clean prospective score sessions show mean non-indeterminate coverage `97.35%`, mean Spearman `0.09305`, mean Top-30 churn `15.3333%`, max `23.3333%`. Structural diagnostics only; no OOS/performance claim and no incumbent/runtime mutation. |
| Alpha available-data research | `ACTIVE` | `codex/alpha-available-data-20260919` | Corrected Stage A complete: C1/C2/C4 retained as structural `FUTURE_RESEARCH`, C3 blocked on sparse/partial PIT coverage; earlier non-conformant implementation attempts retained as failures. Historical alpha claims remain blocked by authoritative Data QA admission; no target/outcome opened. H-FRAG-01 remains `SOURCE_BLOCKED / PARTIAL`; FILINGAGE-01 is `SOURCE_PARTIAL_STRUCTURAL_SIGNAL / NOT_ADMITTED`; EXECSTATE-01 is `SOURCE_PARTIAL_STRUCTURAL_SIGNAL / NOT_ADMITTED`; SUSPSTATE-01 is `SOURCE_BLOCKED` because `120,498/121,666` unique no-trade keys are outside the sparse regular suspension intervals and `471` interval-key groups overlap. The 188-row CA residual remains outside the HLC overlay (`0/188`). SECTOR-01 found `22` structured 2022/2023 sheets and `1,607` rows, but no daily/PIT/identity/vintage authority; it remains `SOURCE_PARTIAL_STRUCTURAL_SIGNAL / NOT_ADMITTED`. Current work: system-deep-dive audit of Decision, sizing, execution, CA/accounting, identity, artifact lineage, reconciliation provenance, exposure/cash-state taxonomy, retained-fixture availability, post-entry risk drift, prospective formula independence, execution-evaluation quantity boundary, and malformed-latest snapshot recovery. New checkpoints `2026-09-20_IDX_RECONCILIATION_FLAG_PROVENANCE_AUDIT_V1`, `2026-09-20_IDX_EXPOSURE_CASH_STATE_TAXONOMY_AUDIT_V1`, `2026-09-20_IDX_LEGACY_OBLIGATION_FIXTURE_INVENTORY_V1`, `2026-09-20_IDX_POST_ENTRY_WEIGHT_DRIFT_AUDIT_V1`, `2026-09-20_IDX_PROSPECTIVE_FORMULA_INDEPENDENCE_AUDIT_V1`, `2026-09-20_IDX_EXECUTION_EVALUATION_QUANTITY_BOUNDARY_AUDIT_V1`, and `2026-09-20_IDX_MALFORMED_LATEST_SNAPSHOT_RECOVERY_AUDIT_V1`; exact runtime probe shows poisoned latest snapshots fail closed but are not quarantined or bypassed by ancestor fallback; runtime/canonical/provider/capture/cloud/R2/scheduler/counter/protected-data mutation remains prohibited. |
