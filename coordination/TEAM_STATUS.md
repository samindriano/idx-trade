...
| Research Integrity / Data QA Gate V1 | `REVIEW` | PR #103 `audit/research-integrity-data-qa-gate-v1@db97ad38`; stacked PR #108 `data/ca-aware-feature-basis-remediation-v1@5f958319` | Phase-1 fail-closed framework validation/hardening passed. INC-001 remains CONFIRMED/NOT_CLOSED. Outcome-blind CA-aware feature-basis primitives are validated and A-D reconciliation is complete, but historical application remains blocked: exact final-fit support is 629 tickers versus the 610-ticker KSEI census; KSEI lacks per-session as-of/no-event attestation; and structural family coverage/transition taxonomy remains partial or contradictory, including MERGER/VOLUNTARY_CONVERSION. R2 also pins the full clean feature-panel observation interval (2021-04-29 through 2026-07-31), confirming the temporal gate cannot be reduced to final-fit support. DATA_ADMISSION=FAIL, RESEARCH_ADMISSION=FAIL, MODEL_PROMOTION=NOT_EVALUATED. Phase-E recompute was not run. No V4-X1 refit, outcome access, counter mutation, capture/runtime change, provider acquisition, historical feature recomputation, or science rescue is authorized. |
...
| E2E Paper cloud-first orchestration | `ACTIVE` | main workflow `.github/workflows/e2e-paper-cloud-orchestration.yml` activation merge `53767b2b`; implementation pin `6e1bf4a1e47a2abff365b35c19687444cf3f0596`; input bridge `ops/e2e-paper-cloud-input-provisioning-v1@13cd07af` | Production workflow now invokes V3. PREOPEN_CA retries run at 08:30/08:45/08:55 WIB, PREOPEN retries remain 09:03/09:13/09:22, and POST_EOD retries remain 18:35/19:05/19:35. PREOPEN_CA, PREOPEN, and POST_EOD use isolated concurrency groups; the V3 09:02 cutoff remains fail-closed. No new production provider/R2 bundle or protected outcome was accessed; the first genuine 2026-08-27 scheduled run is the operational proof. |
...
For E2E cloud-first: observe the first eligible scheduled PREOPEN_CA/PREOPEN/POST_EOD cycle on the repinned main workflow and verify the resulting cloud evidence. Keep Windows fallbacks unchanged unless a separately approved single-writer cutover requires reversible suppression. Do not backfill missed sessions.
