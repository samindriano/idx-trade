...
| 100-session prospective alpha evaluation | `ACTIVE` | `research/idx-forward-evaluation-protocol-v1`; `codex/idx-forward-100-evaluator-v1`; `research/idx-v4-x1-prospective-evaluation-protocol-v1`; draft PR #83 | V4-X1 protocol and deterministic development-safe evaluator are frozen/implemented. User authorized the next pre-outcome step: implement and test a separately committed protected-outcome access gate/runner on synthetic/non-prospective fixtures only. It must verify 100/100 session identity, canonical target maturity/provenance, protocol/evaluator pins, PaperState continuity, benchmark pinning, marker ordering, and one-shot/idempotency before any future vault read. Protected outcomes remain sealed during development. |
