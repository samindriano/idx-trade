...
- **Cloudflare:** current `origin/main` source/bundle/config/profile/package identity is freshly attested; preparation returns `CLOUDFLARE_PRODUCTION_PREPARED_NONAUTOMATIC`, while the active candidate is intentionally not promoted. Windows V1 remains the automatic controller, but stale enabled one-shot tasks are an ACL blocker.
