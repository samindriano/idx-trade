...
- `src/idx_trade/forward_foreign_flow_context_bridge.py`
  ...
  - bridge-only Stock Summary + Yahoo context capture;
  - Foreign Flow normalization through existing accepted parser;
...
- focused tests for calendar isolation, canonical/bridge ambiguity, invalid-canonical fallback, post-monitor canonical-only policy, planner behavior, and operator-counter non-mutation.
...
2. Checkout/pull `data/foreign-flow-forward-context-bridge-v1`.
3. Run focused tests:
...
8. Stop and inspect the planner result before provider calls.
