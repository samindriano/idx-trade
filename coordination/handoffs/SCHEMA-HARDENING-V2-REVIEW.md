...
6. Decimal representation-only scale does not alter canonical hashes/dedup keys.
7. Duplicate holdings and duplicate cash identities are rejected.

## Files

- `src/idx_trade/personal_portfolio/contracts.py`
- `src/idx_trade/personal_portfolio/schema.py`
- `src/idx_trade/personal_portfolio/storage.py`
...
Run the focused tests, compile/import validation, JSON-schema parsing, schema/runtime parity and round-trip checks. Adversarially test completeness evidence, concurrent duplicate append, mutation/deletion rejection, source-pin immutability, opaque subaccount enforcement, sensitive-value minimization, decimal canonicalization, and duplicate positions/cash.
